...
function forecastChartMarkup(){
  ...
  const W=600,H=230,padX=42,padY=32;
  ...
  const guideValues=[max,(max+min)/2,min];
  const guides=guideValues.map(v=>`<g><line x1="${padX}" x2="${W-padX}" y1="${y(v).toFixed(1)}" y2="${y(v).toFixed(1)}" class="forecast-guide"/><text x="0" y="${(y(v)+4).toFixed(1)}" class="forecast-guide-label">${euroFmt.format(v)}</text></g>`).join('');
...
function refreshFinanceControl(control){
  if(!control)return;
  const wrapper=control.parentElement?.querySelector('.finance-control');
  if(wrapper){wrapper.textContent=control.type==='date'?financeDateLabel(control.value):(control.options[control.selectedIndex]?.text||'Seleziona');}
}
...
function enhanceFinanceControls(scope=document){
  scope.querySelectorAll('select,input[type="date"]').forEach(control=>{
    if(control.dataset.financeEnhanced==='1'){refreshFinanceControl(control);return;}
    control.dataset.financeEnhanced='1';
    const button=document.createElement('button');
    button.type='button';
    button.className='finance-control';
    button.textContent=control.tagName==='SELECT'?(control.options[control.selectedIndex]?.text||'Seleziona'):financeDateLabel(control.value);
    control.classList.add('finance-native-hidden');
    control.insertAdjacentElement('afterend',button);
    button.addEventListener('click',()=>control.tagName==='SELECT'?openFinanceSelect(control):openFinanceDate(control));
    control.addEventListener('change',()=>refreshFinanceControl(control));
  });
}
function closeFinancePicker(){document.getElementById('financePickerRoot')?.remove()}
function openFinancePicker(content){
  closeFinancePicker();
  const root=document.createElement('div');root.id='financePickerRoot';root.className='finance-picker-backdrop';
  root.innerHTML=`<div class="finance-picker" onclick="event.stopPropagation()">${content}</div>`;
  root.addEventListener('click',e=>{if(e.target===root)closeFinancePicker()});
  document.body.appendChild(root);
}
function openFinanceSelect(select){
  const options=[...select.options];
  openFinancePicker(`<div class="finance-picker-head"><h2>Seleziona</h2><button type="button" class="close-btn" onclick="closeFinancePicker()">×</button></div><div class="finance-option-list">${options.map((o,i)=>`<button type="button" class="finance-option ${o.selected?'selected':''}" onclick="chooseFinanceOption('${select.id||''}',${i})">${escapeHtml(o.text)}</button>`).join('')}</div>`);
  const picker=document.getElementById('financePickerRoot');
  picker.dataset.selectName=select.name||'';
  picker.dataset.selectId=select.id||'';
  picker._select=select;
}
function chooseFinanceOption(selectId,index){
  const picker=document.getElementById('financePickerRoot');const select=picker?._select||document.getElementById(selectId);if(!select)return;
  select.selectedIndex=index;select.dispatchEvent(new Event('change',{bubbles:true}));refreshFinanceControl(select);closeFinancePicker();
}
function openFinanceDate(input){
  const value=input.value||todayIso();const selected=new Date(value+'T12:00');
  renderFinanceDatePicker(input,selected.getFullYear(),selected.getMonth(),value);
}
function renderFinanceDatePicker(input,year,month,selectedValue){
  const first=new Date(year,month,1), days=new Date(year,month+1,0).getDate(), start=(first.getDay()+6)%7;
  const weekdays=['L','M','M','G','V','S','D'];
  const cells=[];for(let i=0;i<start;i++)cells.push('<span class="finance-day empty"></span>');
  for(let day=1;day<=days;day++){const value=`${year}-${String(month+1).padStart(2,'0')}-${String(day).padStart(2,'0')}`;cells.push(`<button type="button" class="finance-day ${value===selectedValue?'selected':''}" onclick="chooseFinanceDate('${value}')">${day}</button>`)}
  const monthLabel=new Date(year,month,1).toLocaleDateString('it-IT',{month:'long',year:'numeric'});
  openFinancePicker(`<div class="finance-picker-head"><h2>Seleziona data</h2><button type="button" class="close-btn" onclick="closeFinancePicker()">×</button></div><div class="finance-date-current">${financeDateLabel(selectedValue)}</div><div class="finance-calendar-nav"><button type="button" onclick="shiftFinanceDate(-1)">‹</button><b>${monthLabel}</b><button type="button" onclick="shiftFinanceDate(1)">›</button></div><div class="finance-weekdays">${weekdays.map(d=>`<span>${d}</span>`).join('')}</div><div class="finance-calendar-grid">${cells.join('')}</div><div class="finance-picker-actions"><button type="button" class="secondary-btn" onclick="closeFinancePicker()">Annulla</button><button type="button" class="primary-btn" onclick="chooseFinanceDate('${todayIso()}')">Oggi</button></div>`);
  const picker=document.getElementById('financePickerRoot');picker._dateInput=input;picker.dataset.year=String(year);picker.dataset.month=String(month);picker.dataset.selected=selectedValue;
}
function shiftFinanceDate(delta){const picker=document.getElementById('financePickerRoot');if(!picker)return;let year=Number(picker.dataset.year),month=Number(picker.dataset.month)+delta;if(month<0){month=11;year--}if(month>11){month=0;year++}renderFinanceDatePicker(picker._dateInput,year,month,picker.dataset.selected)}
function chooseFinanceDate(value){const picker=document.getElementById('financePickerRoot');const input=picker?._dateInput;if(!input)return;input.value=value;input.dispatchEvent(new Event('change',{bubbles:true}));refreshFinanceControl(input);closeFinancePicker()}
function financeConfirm(message,onConfirm,{title='Conferma',danger=false}={}){openModal(`<div class="confirm-card finance-dialog"><div class="confirm-icon">${danger?'⚠':'?'}</div><h2>${escapeHtml(title)}</h2><p>${escapeHtml(message).replace(/\n/g,'<br>')}</p><div class="modal-actions confirm-actions"><button type="button" class="secondary-btn" onclick="closeModal()">Annulla</button><button type="button" class="${danger?'danger-btn':'primary-btn'}" id="financeConfirmAction">Conferma</button></div></div>`);document.getElementById('financeConfirmAction').onclick=()=>{closeModal();onConfirm()}}
function financeAlert(message,{title='Avviso'}={}){openModal(`<div class="confirm-card finance-dialog"><div class="confirm-icon">!</div><h2>${escapeHtml(title)}</h2><p>${escapeHtml(message).replace(/\n/g,'<br>')}</p><div class="modal-actions confirm-actions"><button type="button" class="primary-btn" onclick="closeModal()">OK</button></div></div>`)}
...
function submitTransaction(ev){
  ...
  if(canonicalCategory(data.category)==='Obiettivi'&&!description){financeAlert('Per un movimento Obiettivi la descrizione è obbligatoria.');return;}
  ...
  if(match){
    ...
    openModal(`<div class="confirm-card finance-dialog"><div class="confirm-icon">?</div><h2>Possibile evento futuro trovato</h2><p>Hai appena registrato: <b>${escapeHtml(transaction.description||transaction.category)} — ${euroFmt.format(transaction.amount)}</b><br><br>Potrebbe corrispondere all'evento futuro: <b>${escapeHtml(event.description)} — ${euroFmt.format(event.amount)}</b><br><br>Era per caso l'evento futuro che stavamo aspettando?</p><div class="modal-actions confirm-actions"><button type="button" class="secondary-btn" id="futureEventNo">No, è un altro movimento</button><button type="button" class="primary-btn" id="futureEventYes">Sì, conferma</button></div></div>`);
    document.getElementById('futureEventNo').onclick=finishTransactionSave;
    document.getElementById('futureEventYes').onclick=()=>{state.futureEvents=state.futureEvents.filter(e=>e.id!==event.id);finishTransactionSave()};
    return;
  }
  finishTransactionSave();
}
function finishTransactionSave(){recalculateBalance();save();closeModal();render()}
...
function importBackup(event){
  ...
  reader.onload=()=>{
    try{
      ...
      financeConfirm('Importando il backup i dati attuali verranno sostituiti. Vuoi continuare?',()=>applyImportedBackup(imported),{title:'Importare il backup?',danger:true});return;

    }catch(err){
      financeAlert('Impossibile importare il backup: il file non è valido.',{title:'Importazione non riuscita'});
      console.warn('Errore importazione backup:',err);
    }
  };
  reader.readAsText(file);
}
function applyImportedBackup(imported){const safe={...structuredClone(DEFAULT_STATE),...imported,transactions:Array.isArray(imported.transactions)?imported.transactions:[],futureEvents:Array.isArray(imported.futureEvents)?imported.futureEvents:[],goals:Array.isArray(imported.goals)?imported.goals:[],cushionMinimum:Math.max(0,Number(imported.cushionMinimum)||0),cushionIdeal:Math.max(0,Number(imported.cushionIdeal)||0)};Object.keys(state).forEach(key=>delete state[key]);Object.assign(state,safe);recalculateBalance();save();closeModal();render();setTimeout(()=>financeAlert('Backup importato correttamente.',{title:'Importazione completata'}),0)}
...
function resetDemo(){financeConfirm('Vuoi ripristinare i dati iniziali?',()=>{localStorage.removeItem('matteos-finance-v1');location.reload()},{title:'Ripristinare i dati?',danger:true})}
