...
function monthInsightButtons(){
  ...
  if(months.length<2){
    ...
    return `<button class="big-btn insight-locked" disabled>Disponibile tra<br><small class="insight-size-spacer">In attesa di dati</small></button><button class="big-btn insight-locked" disabled>${days} giorni<br><small class="insight-size-spacer">In attesa di dati</small></button>`;
  }
  ...
  return `<button class="big-btn income insight-ready" onclick="openMonthAnalysis('${best.m}')">Mese migliore<br><small class="insight-size-spacer">${monthName(best.m)} · ${best.net>=0?'+':''}${euroFmt.format(best.net)}</small></button><button class="big-btn expense insight-ready" onclick="openMonthAnalysis('${worst.m}')">Mese peggiore<br><small class="insight-size-spacer">${monthName(worst.m)} · ${worst.net>=0?'+':''}${euroFmt.format(worst.net)}</small></button>`;
}
