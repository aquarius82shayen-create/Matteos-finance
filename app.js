...
function realCategoryTotals(ym,type){
  const byCategory={};
  monthTx(ym).filter(t=>t.type===type).forEach(t=>{
...
function realMonthTotals(ym){
  const incomeByCategory=realCategoryTotals(ym,'income');
  const expenseByCategory=realCategoryTotals(ym,'expense');
  return {
    income:sum(Object.values(incomeByCategory)),
    expenses:sum(Object.values(expenseByCategory)),
    incomeByCategory,
    expenseByCategory
  };
}
function remainingAverageByCategory(ym,type){
  const average=averageCategoryTotals(ym,type).byCategory;
  if(ym!==currentMonth())return average;
  const actual=realCategoryTotals(ym,type);
  const remaining={};
  Object.keys(average).forEach(category=>{
    // Il dato reale già registrato viene prima di qualsiasi stima: per evitare
    // duplicazioni si prevede solo la parte eventualmente ancora mancante.
    remaining[category]=Math.max(0,(Number(average[category])||0)-(Number(actual[category])||0));
  });
  return remaining;
}
...
function normalForecastForMonth(ym){
  ...
  const incomeByCategory=remainingAverageByCategory(ym,'income');
  const expenseByCategory=remainingAverageByCategory(ym,'expense');
  return {
    income:sum(Object.values(incomeByCategory)),
    expenses:sum(Object.values(expenseByCategory)),
    incomeByCategory,
