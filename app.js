...
function dashboardMonthDays(ym=currentMonth()){
...
return `<section class="page dashboard-page ${state.page==='dashboard'?'active':''}">
  <div class="hero">
    ...
    <button type="button" class="month-select finance-choice-trigger" aria-label="Mese dashboard" onclick="openDashboardMonthPicker()">${monthName(ym)} <span>▾</span></button>
  ...
  <div class="section"><div class="section-title week-title"><span>${new Date(currentMonth()+'-01T12:00:00').toLocaleDateString('it-IT',{month:'long'}).toUpperCase()}</span></div><div class="panel week-panel"><div class="week-viewport"><div class="week-grid month-grid">${days.map(day=>dayCardMarkup(day,day.key===todayIso())).join('')}</div></div></div></div>
...
function openFinanceChoice(title,options,selected,onSelect){const root=pickerRoot();root.innerHTML=`<div class="finance-picker-backdrop" onclick="if(event.target===this)closeFinancePicker()"><div class="finance-picker-card finance-select-picker"><div class="finance-picker-head"><div><small>Matteo's Finance</small><h2>${escapeHtml(title)}</h2></div><button type="button" class="finance-picker-close" onclick="closeFinancePicker()">×</button></div><div class="finance-option-list">${options.map((value,i)=>`<button type="button" class="finance-option ${value===selected?'selected':''}" data-choice-index="${i}"><span>${escapeHtml(value)}</span><i>${value===selected?'✓':''}</i></button>`).join('')}</div></div></div>`;root.querySelectorAll('[data-choice-index]').forEach(btn=>btn.onclick=()=>{const value=options[Number(btn.dataset.choiceIndex)];closeFinancePicker();onSelect(value)})}
function openDashboardMonthPicker(){openFinanceChoice('Seleziona mese',availableDataMonths(),state.dashboardMonth,setDashboardMonth)}
function openMovementMonthPicker(){const options=['Tutti i mesi',...availableDataMonths().map(m=>monthName(m))];const selected=state.movementMonth==='all'?'Tutti i mesi':monthName(state.movementMonth);openFinanceChoice('Seleziona mese',options,selected,label=>{const index=options.indexOf(label);setMovementMonth(index===0?'all':availableDataMonths()[index-1])})}
function forecastMonthLabel(ym){return new Date(ym+'-01T12:00:00').toLocaleDateString('it-IT',{month:'long',year:'2-digit'})}
function openForecastMonthPicker(){const months=monthOptions();const options=months.map(forecastMonthLabel);const selected=forecastMonthLabel(state.forecastMonth);openFinanceChoice('Mese di riferimento',options,selected,label=>{state.forecastMonth=months[options.indexOf(label)];save();render();requestAnimationFrame(()=>window.scrollTo({top:0,left:0,behavior:'auto'}))})}
...
async function exportBackup(){
  ...
  const filename='matteos-finance-backup.json';
  if('showSaveFilePicker' in window){
    try{
      const handle=await window.showSaveFilePicker({suggestedName:filename,types:[{description:'Backup Matteo’s Finance',accept:{'application/json':['.json']}}]});
