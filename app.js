...
return `<section class="page dashboard-page ${state.page==='dashboard'?'active':''}${dashboardWeekTransition?' week-changing':''}">
  <div class="hero">
    <img class="dashboard-banner" src="dashboard-header.png" alt="Matteo's Finance" />
...
function futureEventsConsultationMarkup(){
  return `<button type="button" class="panel future-events-consult-card" onclick="openFutureEventsPage()" aria-label="Tutti gli eventi futuri"><span>TUTTI GLI EVENTI FUTURI</span><b>›</b></button>`;
}
function openFutureEventsPage(){
  state.page='future-events';
  save();
  render();
  requestAnimationFrame(()=>window.scrollTo({top:0,left:0,behavior:'auto'}));
}
function futureEventsPage(){
  const rows=futureEventsConsultation().map(e=>`<div class="future-event-consult-row"><span class="future-event-consult-date">${escapeHtml(new Date(e.date+'T12:00:00').toLocaleDateString('it-IT',{day:'numeric',month:'long',year:'numeric'}))}</span><span class="future-event-consult-description">${escapeHtml(e.description||'Evento')}</span><span class="future-event-consult-category">${escapeHtml(e.category||'')}</span><strong class="${e.type==='income'?'positive':'negative'}">${e.type==='income'?'+':'-'}${euroFmt.format(Number(e.amount)||0)}</strong></div>`).join('');
  return `<section class="page ${state.page==='future-events'?'active':''}"><div class="page-head future-events-page-head"><button type="button" class="icon-btn" onclick="changePage('forecasts')" aria-label="Torna a Previsioni">‹</button><div><h1>Tutti gli eventi futuri</h1></div><div></div></div><div class="future-events-consult-list">${rows||'<div class="empty">Nessun evento futuro programmato.</div>'}</div></section>`;
}
