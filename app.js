...
return `<section class="page ${state.page==='dashboard'?'active':''}">
  <div class="hero">
    ...
    <div class="balance-card"><div class="balance-label">TOTALE DISPONIBILE</div><div class="balance-value">${euroFmt.format(state.balance)}</div><div class="balance-delta"><span class="positive">↗ ${euroFmt.format(sav)}</span> &nbsp; saldo del mese</div><div class="balance-buttons"><button class="big-btn income" onclick="openTransaction('income')">⊕ &nbsp; Aggiungi entrata</button><button class="big-btn expense" onclick="openTransaction('expense')">↟ &nbsp; Aggiungi uscita</button></div></div>
  </div>
