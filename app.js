...
return `<section class="page dashboard-page ${state.page==='dashboard'?'active':''}${dashboardWeekTransition?' week-changing':''}">
  <div class="hero">
    ...
    <div class="balance-card"><div class="balance-label-row"><div class="balance-label">TOTALE DISPONIBILE</div></div><div class="balance-value-row"><div class="balance-value">${euroFmt.format(balanceAtEndOfMonth(ym))}</div><img class="balance-wallet" src="assets/wallet-transparent.png" alt="Portafoglio" /></div><div class="balance-delta"><span class="positive">↗ ${euroFmt.format(sav)}</span> &nbsp; saldo del mese</div><div class="balance-buttons"><button class="big-btn income" onclick="openTransaction('income')">⊕ &nbsp; Aggiungi entrata</button><button class="big-btn expense" onclick="openTransaction('expense')">↟ &nbsp; Aggiungi uscita</button></div></div>
  </div>
...
function normalizedDescription(value=''){return String(value).trim().toLocaleLowerCase('it-IT').replace(/\s+/g,' ')}
...
function normalForecastForMonth(ym){
  ...
  const actualByType={income:new Set(),expense:new Set()};
  if(ym===currentMonth())monthTx(ym).forEach(t=>actualByType[t.type]?.add(t.category));
  const forecastType=type=>{
    const avg=averageCategoryTotals(ym,type);
    let total=0;
    Object.entries(avg.byCategory).forEach(([category,value])=>{
      if(ym===currentMonth()&&actualByType[type].has(category))return;
      total+=value*factor;
    });
...
function analyseGoal(g){
  ...
  const end=new Date(g.deadline+'T12:00');
  const deadlineMonth=`${end.getFullYear()}-${pad2(end.getMonth()+1)}`;
  let months=(Number(deadlineMonth.slice(0,4))-ry)*12+(Number(deadlineMonth.slice(5,7))-rm)+1;
  if(!Number.isFinite(months))months=0;
  months=Math.max(0,months);
  const needed=months>0?Math.max(0,(g.cost-g.saved)/months):Math.max(0,g.cost-g.saved);
  const cap=Math.max(0,monthlyCapacity());
  const ratio=cap?needed/cap:Infinity;let status='good',label='✓ In linea';if(ratio>.9&&ratio<=1.25){status='watch';label='⚠ Da monitorare'}if(ratio>1.25){status='bad';label='✕ Non raggiungibile'}return {monthsLeft:months,needed,capacity:cap,status,label,ratio}
}
function movements(){
  ...
  return `<section class="page ${state.page==='movements'?'active':''}"><div class="page-head"><div><h1>Movimenti</h1></div><button class="primary-btn" onclick="openTransaction('income')">＋</button></div><div class="month-control movement-month-control"><select onchange="setMovementMonth(this.value)"><option value="all" ${state.movementMonth==='all'?'selected':''}>Tutti i mesi</option>${months.map(m=>`<option value="${m}" ${m===state.movementMonth?'selected':''}>${monthName(m)}</option>`).join('')}</select></div><div class="filters"><button class="filter-btn ${state.movementFilter==='all'?'active':''}" onclick="setFilter('all')">Tutti</button><button class="filter-btn ${state.movementFilter==='income'?'active':''}" onclick="setFilter('income')">Entrate</button><button class="filter-btn ${state.movementFilter==='expense'?'active':''}" onclick="setFilter('expense')">Uscite</button></div><div class="movement-list">${items.map(t=>`<div class="movement-row"><div class="event-icon">${t.type==='income'?'↗':'↘'}</div><div class="event-meta"><b>${escapeHtml(t.description||t.category)}</b><small>${t.category} · ${new Date(t.date+'T12:00').toLocaleDateString('it-IT')}</small></div><div class="event-actions"><strong class="movement-amount ${t.type==='income'?'positive':'negative'}">${t.type==='income'?'+':'-'}${euroFmt.format(t.amount)}</strong><button class="mini-btn delete" onclick="deleteTransaction('${t.id}')">⌫</button></div></div>`).join('')||'<div class="empty">Nessun movimento.</div>'}</div></section>`
}
...
function submitTransaction(ev){ev.preventDefault();const f=ev.currentTarget,d=new FormData(f),amount=Number(d.get('amount')),description=String(d.get('description')||'').trim();state.transactions.push({id:uid(),date:d.get('date'),type:f.dataset.type,category:d.get('category'),description,amount});recalculateBalance();save();closeModal();render()}
