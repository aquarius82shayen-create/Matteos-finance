...
/* Motore icone: 1) nomi/entita specifiche; 2) luoghi; 3) concetti ricavati dai nomi .webp; 4) freccia. */
function semanticIconFile(description='',category='',type=''){
  ...
  const terms=f=>label(f).split(' ').filter(w=>w.length>2&&!new Set(['e','a','ad','al','alla','alle','ai','agli','da','dal','dalla','di','del','della','dei','degli','in','per','con','su','il','lo','la','i','gli','le','un','una','uno','the','and','of','to']).has(w));
  const byId=new Map(ICON_LIBRARY_FILES.map(f=>[idOf(f),f]));

  // 1. Nomi/marchi/artisti specifici. L'ordine nella descrizione risolve i pari livello.
  const namedIds=[190,191,192,194]; const named=[];
  for(const id of namedIds){const f=byId.get(id);if(!f)continue;let aliases=[label(f)];if(id===191)aliases.push('mcdonald',"mcdonald's",'mc donald');if(id===192)aliases.push('deliveroo');if(id===194)aliases.push('iron maiden');if(id===190)aliases.push('puttana','puttane','mignotte','pompino');const hits=aliases.map(x=>({x,pos:position(x)})).filter(x=>x.pos<Infinity);if(hits.length){const h=hits.sort((a,b)=>a.pos-b.pos||b.x.length-a.x.length)[0];named.push({file:f,pos:h.pos,len:normalize(h.x).length});}}
  if(named.length){named.sort((a,b)=>a.pos-b.pos||b.len-a.len);return named[0].file;}

  // 2. Paesi/localita: prevalgono sui concetti generici.
  ...
  // 3. Usa i nomi descrittivi dei file come vocabolario. Le corrispondenze multiple
  // rendono una icona piu specifica; le frasi piu lunghe prevalgono sui singoli termini.
  const candidates=[];
  for(const f of ICON_LIBRARY_FILES){if(namedIds.includes(idOf(f)))continue;const ts=terms(f);const hits=ts.filter(t=>new RegExp(`(^|\\s)${t.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}(?=\\s|$)`,'u').test(text));if(!hits.length)continue;let phrase=0;for(let i=0;i<ts.length;i++)for(let j=i+2;j<=Math.min(ts.length,i+4);j++){const p=ts.slice(i,j).join(' ');if(p.length>3&&text.includes(p))phrase=Math.max(phrase,p.length);}candidates.push({file:f,count:hits.length,phrase,pos:Math.min(...hits.map(position)),id:idOf(f)});}
  if(candidates.length){candidates.sort((a,b)=>b.count-a.count||b.phrase-a.phrase||a.pos-b.pos||a.id-b.id);return candidates[0].file;}
  return '';
}
