...
function goalForecastMonths(g){
  const months=monthOptions();
  const start=Math.max(0,months.indexOf(currentMonth()));
  const deadlineMonth=monthOf(g.deadline);
  let end=months.indexOf(deadlineMonth);
  if(end<start) end=start;
  if(end<0) end=months.length-1;
  return months.slice(start,end+1);
}
function forecastCapacityForGoal(g){
  const months=goalForecastMonths(g);
  const cushion=Math.max(0,Number(g.cushion)||0);
  const currentBalance=Number(state.balance)||0;
  const currentFree=Math.max(0,currentBalance-cushion);
  const projections=months.map(m=>forecast(m));
  const lastProjection=projections[projections.length-1]||{endBalance:currentBalance};
  const forecastEnd=Math.max(0,Number(lastProjection.endBalance)||0);
  const freeAtDeadline=Math.max(0,forecastEnd-cushion);
  // La capacità dell'obiettivo deriva dalla stessa traiettoria delle Previsioni.
  // Il cuscino resta sempre intatto: prima viene mantenuta la soglia, poi il
  // denaro eccedente può aumentare la disponibilità dell'obiettivo.
  const availableGrowth=Math.max(0,freeAtDeadline-currentFree);
  const monthlyCapacity=availableGrowth/Math.max(1,months.length);
  return {months,projections,cushion,currentFree,forecastEnd,freeAtDeadline,availableGrowth,monthlyCapacity};
}
function analyseGoal(g){
  const forecastData=forecastCapacityForGoal(g);
  const months=Math.max(1,forecastData.months.length);
  const needed=Math.max(0,(g.cost-(Number(g.saved)||0))/months);
  const cap=forecastData.monthlyCapacity;
  const ratio=cap?needed/cap:(needed===0?0:Infinity);
  let status='good',label='✓ In linea';
  if(ratio>.9&&ratio<=1.25){status='watch';label='⚠ Da monitorare'}
  if(ratio>1.25){status='bad';label='✕ Non raggiungibile'}
  return {...forecastData,monthsLeft:months,needed,capacity:cap,status,label,ratio}
}
function movements(){const items=[...state.transactions].sort((a,b)=>b.date.localeCompare(a.date)).filter(t=>state.movementFilter==='all'||t.type===state.movementFilter);return `<section class="page ${state.page==='movements'?'active':''}"><div class="page-head"><div><h1>Movimenti</h1></div><button class="primary-btn" onclick="openTransaction('income')">＋</button></div><div class="filters"><button class="filter-btn ${state.movementFilter==='all'?'active':''}" onclick="setFilter('all')">Tutti</button><button class="filter-btn ${state.movementFilter==='income'?'active':''}" onclick="setFilter('income')">Entrate</button><button class="filter-btn ${state.movementFilter==='expense'?'active':''}" onclick="setFilter('expense')">Uscite</button></div><div class="movement-list">${items.map(t=>`<div class="movement-row"><div class="event-icon">${t.type==='income'?'↗':'↘'}</div><div class="event-meta"><b>${escapeHtml(t.description)}</b><small>${t.category} · ${new Date(t.date+'T12:00').toLocaleDateString('it-IT')}</small></div><div class="event-actions"><strong class="movement-amount ${t.type==='income'?'positive':'negative'}">${t.type==='income'?'+':'-'}${euroFmt.format(t.amount)}</strong><button class="mini-btn delete" onclick="deleteTransaction('${t.id}')">⌫</button></div></div>`).join('')||'<div class="empty">Nessun movimento.</div>'}</div></section>`}
function render(){
