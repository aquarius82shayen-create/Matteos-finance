...
const ICONS={
  travel:'1000172916.webp', flight:'1000172918.webp', beach:'1000172919.webp', mountain:'1000172920.webp', hotel:'1000172921.webp', camping:'1000172922.webp', concert:'1000172924.webp', bar:'1000172926.webp', theatre:'1000172927.webp', gym:'1000172941.webp', car:'1000172942.webp', repair:'1000172943.webp', carIssue:'1000172944.webp', fuel:'1000172945.webp', driving:'1000172946.webp', train:'1000172947.webp', cruise:'1000172948.webp', house:'1000172949.webp', appliances:'1000172950.webp', gas:'1000172951.webp', light:'1000172952.webp', internet:'1000172953.webp', bills:'1000172954.webp', charity:'1000172955.webp', moving:'1000172956.webp', tools:'1000172957.webp', gift:'1000172958.webp', love:'1000172980.webp', health:'1000172981.webp', doctor:'1000172982.webp', dentist:'1000172983.webp', family:'1000172984.webp', healthHeart:'1000172985.webp', mushrooms:'1000172986.webp', pet:'1000172987.webp', vet:'1000172988.webp', pharmacy:'1000172989.webp', sunglasses:'1000172990.webp', shopping:'1000172992.webp', electronics:'1000172996.webp', house2:'1000172997.webp', target:'1000172998.webp', wallet:'1000172999.webp', party:'1000173000.webp', holiday:'1000173001.webp', christmas:'1000173002.webp', newYear:'1000173003.webp', easter:'1000173004.webp', ferragosto:'1000173005.webp', dinner:'1000173006.webp', aperitivo:'1000173008.webp', restaurant:'1000173011.webp', wedding:'1000173012.webp', mourning:'1000173013.webp', taxes:'1000173014.webp',
  ...
  stadio:'138_stadio.webp', sport:'139_sport.webp', museo:'140_museo.webp', pallavolo:'141_pallavolo.webp', calcio:'142_calcio.webp', basket:'143_basket.webp', surf:'144_surf.webp', bilancia:'145_bilancia.webp', rugby:'146_rugby.webp', libri:'147_libri.webp', musica:'148_musica.webp', aperitivoNamed:'149_aperitivo.webp', pizza:'150_pizza.webp', cucinaCinese:'151_cucina_cinese.webp', hamburger:'152_hamburger.webp', irlanda:'153_irlanda.webp', uk:'1000173317.webp'
};
...
function semanticIconFile(description='',category='',type='expense'){
  const x=normalizeIconText(`${description} ${category}`);
  // Prima le situazioni più specifiche; poi luogo, attività e categoria.
  if(/iron maiden|metal|heavy metal/.test(x)) return ICONS.musica;
  if(/concerto|live music|festival musicale/.test(x)) return ICONS.concert;
  if(/chitarra|musica|strumento musicale|spotify/.test(x)) return ICONS.musica;
  if(/aperitivo/.test(x)) return ICONS.aperitivoNamed;
  if(/pizza|pizzeria/.test(x)) return ICONS.pizza;
  if(/cucina cinese|cinese|ristorante cinese/.test(x)) return ICONS.cucinaCinese;
  if(/hamburger|burger/.test(x)) return ICONS.hamburger;
  if(/cena|pranzo|ristorante|trattoria|pasto/.test(x)) return ICONS.restaurant;
  if(/supermercato|spesa|alimentari|carrello/.test(x)) return ICONS.shopping;
  if(/regalo|regali|compleanno|natale regalo/.test(x)) return ICONS.gift;
  if(/matrimonio|nozze|spos/.test(x)) return ICONS.wedding;
  if(/natale|christmas/.test(x)) return ICONS.christmas;
  if(/capodanno|anno nuovo/.test(x)) return ICONS.newYear;
  if(/pasqua|easter/.test(x)) return ICONS.easter;
  if(/ferragosto/.test(x)) return ICONS.ferragosto;
  if(/festa|party|discoteca|serata/.test(x)) return ICONS.party;
  if(/revisione|tagliando|officina|meccanico|riparazione/.test(x)) return ICONS.repair;
  if(/benzina|carburante|rifornimento|diesel/.test(x)) return ICONS.fuel;
  if(/assicurazione.*auto|auto.*assicurazione|auto|macchina|veicolo/.test(x)) return ICONS.car;
  if(/casa|affitto|mutuo|appartamento/.test(x)) return ICONS.house;
  if(/trasloco|arredamento|mobili/.test(x)) return ICONS.moving;
  if(/elettrodomestici|lavatrice|frigorifero|forno/.test(x)) return ICONS.appliances;
  if(/bolletta|luce|elettricita|elettricita/.test(x)) return ICONS.light;
  if(/gas/.test(x)) return ICONS.gas;
  if(/internet|fibra|wifi|telefono|cellulare/.test(x)) return ICONS.internet;
  if(/tasse|tributi|f24|dichiarazione|redditi/.test(x)) return ICONS.taxes;
  if(/stipendio|salario|paga|bonifico ricevuto|entrata/.test(x)) return ICONS.wallet;
  if(/credito|rimborso|rimborso spese|cashback/.test(x)) return ICONS.wallet;
  // I luoghi specifici hanno precedenza sulle attività generiche:
  // un weekend a Dublino resta Dublino, mentre un concerto a Dublino resta musica.
  if(/londra|london|inghilterra|uk|regno unito/.test(x)) return ICONS.uk;
  if(/dublino|dublin|irlanda/.test(x)) return ICONS.irlanda;
  if(/berlin|berlino|germania|deutschland/.test(x)) return ICONS.germania;
  if(/parigi|paris|francia/.test(x)) return ICONS.francia;
  if(/rom[aà]|italia/.test(x)) return ICONS.italia;
  if(/barcellona|madrid|spagna/.test(x)) return ICONS.spagna;
  if(/lisbona|portogallo/.test(x)) return ICONS.portogallo;
  if(/new york|usa|america|stati uniti/.test(x)) return ICONS.usa;
  if(/tokyo|tokio|giappone/.test(x)) return ICONS.giappone;
  if(/grecia|atene/.test(x)) return ICONS.grecia;
  if(/turchia|istanbul/.test(x)) return ICONS.turchia;
  if(/bruxelles|belgio/.test(x)) return ICONS.belgio;
  if(/svizzera/.test(x)) return ICONS.svizzera;
  if(/austria|vienna/.test(x)) return ICONS.austria;
  if(/croazia/.test(x)) return ICONS.croazia;
  if(/slovenia/.test(x)) return ICONS.slovenia;
  if(/malta/.test(x)) return ICONS.malta;
  if(/polonia|varsavia/.test(x)) return ICONS.polonia;
  if(/norvegia/.test(x)) return ICONS.norvegia;
  if(/svezia/.test(x)) return ICONS.svezia;
  if(/danimarca/.test(x)) return ICONS.danimarca;
  if(/finlandia/.test(x)) return ICONS.finlandia;
  if(/islanda/.test(x)) return ICONS.islanda;
  if(/romania/.test(x)) return ICONS.romania;
  if(/bulgaria/.test(x)) return ICONS.bulgaria;
  if(/ungheria/.test(x)) return ICONS.ungheria;
  if(/repubblica ceca|praga|cechia/.test(x)) return ICONS.repubblicaCeca;
  if(/slovacchia/.test(x)) return ICONS.slovacchia;
  if(/albania/.test(x)) return ICONS.albania;
  if(/estonia/.test(x)) return ICONS.estonia;
  if(/lettonia/.test(x)) return ICONS.lettonia;
  if(/lituania/.test(x)) return ICONS.lituania;
  if(/messico/.test(x)) return ICONS.messico;
  if(/viaggio|vacanza|weekend|tour|trasferta/.test(x)) return ICONS.travel;
  if(/aereo|volo|volare/.test(x)) return ICONS.flight;
  if(/hotel|albergo|b&b|alloggio/.test(x)) return ICONS.hotel;
  if(/campeggio|tenda/.test(x)) return ICONS.camping;
  if(/mare|spiaggia|isola/.test(x)) return ICONS.beach;
  if(/montagna|trekking|escursione/.test(x)) return ICONS.mountain;
  if(/treno|ferrovia/.test(x)) return ICONS.train;
  if(/nave|crociera|traghetto/.test(x)) return ICONS.cruise;
  if(/medico|salute|farmacia|dentista|ospedale/.test(x)) return /dentista/.test(x)?ICONS.dentist:/farmacia/.test(x)?ICONS.pharmacy:ICONS.doctor;
  if(/animale|cane|gatto|veterinario/.test(x)) return /veterinario/.test(x)?ICONS.vet:ICONS.pet;
  if(/palestra|fitness|sport|allenamento/.test(x)) return ICONS.gym;
  if(/libro|libri|lettura|corso|studio|master/.test(x)) return ICONS.libri;
  if(/museo|mostra|arte/.test(x)) return ICONS.museo;
  if(/calcio/.test(x)) return ICONS.calcio;
  if(/basket/.test(x)) return ICONS.basket;
  if(/pallavolo|volley/.test(x)) return ICONS.pallavolo;
  if(/rugby/.test(x)) return ICONS.rugby;
  if(/surf/.test(x)) return ICONS.surf;
  if(/stadio/.test(x)) return ICONS.stadio;
  if(/sport/.test(x)) return ICONS.sport;
  if(category==='Obiettivi') return ICONS.target;
  if(category==='Auto') return ICONS.car;
  if(category==='Casa'||category==='Affitto') return ICONS.house;
  if(category==='Salute') return ICONS.health;
  if(category==='Tempo libero') return ICONS.party;
  if(category==='Regali') return ICONS.gift;
  if(category==='Spesa') return ICONS.shopping;
  return type==='income'?ICONS.wallet:ICONS.shopping;
}
function eventIcon(description='',category='',type='expense'){
  return iconImg(semanticIconFile(description,category,type));
}
