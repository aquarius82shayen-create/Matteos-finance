...
function bestWorstMonthCards(){
  const months=[...new Set(state.transactions.map(t=>monthOf(t.date)))].filter(m=>m<currentMonth()).sort();
  if(!months.length)return `<div class="best-worst-grid"><div class="month-insight-card green"><small>MESE MIGLIORE</small><b>Ancora nessun mese concluso</b></div><div class="month-insight-card red"><small>MESE PIÙ DIFFICILE</small><b>Ancora nessun mese concluso</b></div></div>`;
  const ranked=months.map(m=>({m,net:savings(m)}));
  const best=ranked.reduce((a,b)=>b.net>a.net?b:a,ranked[0]);
  const worst=ranked.reduce((a,b)=>b.net<a.net?b:a,ranked[0]);
  const card=(kind,item,label)=>`<button class="month-insight-card ${kind}" onclick="openMonthAnalysis('${item.m}')"><small>${label}</small><strong>${monthName(item.m)}</strong><b>${item.net>=0?'+':''}${euroFmt.format(item.net)}</b><span>Analizza il mese ›</span></button>`;
  return `<div class="best-worst-grid">${card('green',best,'MESE MIGLIORE')}${card('red',worst,'MESE PIÙ DIFFICILE')}</div>`;
}
function openMonthAnalysis(ym){
  const tx=monthTx(ym), inc=income(ym), out=expenses(ym), net=inc-out;
  const previous=[...new Set(state.transactions.map(t=>monthOf(t.date)))].filter(m=>m<ym).sort();
  const avgInc=previous.length?sum(previous.map(income))/previous.length:inc;
  const avgOut=previous.length?sum(previous.map(expenses))/previous.length:out;
  const cats={}; tx.filter(t=>t.type==='expense').forEach(t=>cats[canonicalCategory(t.category)]=(cats[canonicalCategory(t.category)]||0)+Number(t.amount||0));
  const top=Object.entries(cats).sort((a,b)=>b[1]-a[1])[0];
  const lines=[];
  if(net>=0) lines.push(`Un mese decisamente positivo: hai chiuso con un saldo di <b>${euroFmt.format(net)}</b>.`); else lines.push(`Un mese più impegnativo, chiuso con <b>${euroFmt.format(net)}</b>, ma utile per capire dove intervenire.`);
  if(inc>avgInc*1.1) lines.push(`Le entrate sono state sopra la tua media recente: un bel contributo al risultato.`);
  else if(inc<avgInc*.9) lines.push(`Le entrate sono state sotto la media recente, quindi il risultato non dipende solo dalle spese.`);
  if(out>avgOut*1.15) lines.push(`Le uscite sono state insolitamente alte rispetto ai mesi precedenti.`);
  else if(out<avgOut*.85) lines.push(`Hai contenuto bene le uscite rispetto alla tua media recente.`);
  if(top) lines.push(`La categoria che ha inciso di più è stata <b>${top[0]}</b> con ${euroFmt.format(top[1])}.`);
  if(net>=0) lines.push(`Da portarsi dietro: prova a ripetere le scelte che hanno funzionato, senza dare per scontato che ogni mese sia identico.`);
  else lines.push(`Niente drammi: individuiamo ciò che è stato eccezionale e ciò che possiamo migliorare, un mese alla volta.`);
  const by=(type)=>Object.entries(tx.filter(t=>t.type===type).reduce((a,t)=>{const c=canonicalCategory(t.category);a[c]=(a[c]||0)+Number(t.amount||0);return a},{})).sort((a,b)=>b[1]-a[1]);
  const rows=(arr,cls)=>arr.map(([c,v])=>`<div class="forecast-row"><span>${c}</span><b class="${cls}">${euroFmt.format(v)}</b></div>`).join('')||'<div class="empty">Nessuna voce</div>';
  openModal(`<div class="modal-head"><h2>Analisi di ${monthName(ym)}</h2><button class="close-btn" onclick="closeModal()">×</button></div><div class="analysis-box" style="margin-top:14px">${lines.slice(0,6).map(x=>`<p>${x}</p>`).join('')}<div class="month-analysis-total ${net>=0?'positive':'negative'}">${net>=0?'↗':'↘'} ${euroFmt.format(net)}</div></div><div class="month-analysis-summary"><h3>Entrate · ${euroFmt.format(inc)}</h3>${rows(by('income'),'positive')}<h3>Uscite · ${euroFmt.format(out)}</h3>${rows(by('expense'),'negative')}</div><div class="modal-actions"><button class="primary-btn" onclick="closeModal()">Chiudi</button></div>`);
}
...
return `<section class="page dashboard-page ${state.page==='dashboard'?'active':''}${dashboardWeekTransition?' week-changing':''}">
  <div class="hero">
    ...
    <div class="balance-card"><div class="balance-label-row"><div class="balance-label">TOTALE DISPONIBILE</div></div><div class="balance-value-row"><div class="balance-value">${euroFmt.format(balanceAtEndOfMonth(ym))}</div><img class="balance-wallet" src="assets/wallet-transparent.png" alt="Portafoglio" /></div><div class="balance-delta"><span class="${sav>=0?'positive':'negative'}">${sav>=0?'↗':'↘'} ${euroFmt.format(sav)}</span> &nbsp; saldo del mese</div>${bestWorstMonthCards()}</div>
  </div>
...
function forecastExpenseCategories(ym){
  const out={}; const factor=remainingMonthFraction(ym);
  if(ym===currentMonth()) monthTx(ym).filter(t=>t.type==='expense').forEach(t=>{const c=canonicalCategory(t.category);out[c]=(out[c]||0)+Number(t.amount||0)});
  const avg=averageCategoryTotals(ym,'expense');
  Object.entries(avg.byCategory).forEach(([c,v])=>out[c]=(out[c]||0)+Number(v||0)*factor);
  futureEventsForMonth(ym).filter(e=>e.type==='expense'&& !eventAlreadyRecorded(e) && (ym!==currentMonth()||e.date>=todayIso())).forEach(e=>{const c=canonicalCategory(e.category||e.description||'Altro');out[c]=(out[c]||0)+Number(e.amount||0)});
  return Object.entries(out).filter(([,v])=>v>.01).sort((a,b)=>b[1]-a[1]);
}
function forecastGraphData(){return monthOptions().filter(m=>m>=currentMonth()).slice(0,6).map(m=>({m,value:forecast(m).endBalance}))}
function forecastGraphMarkup(){
  const data=forecastGraphData(); if(!data.length)return '<div class="empty">Dati insufficienti per il grafico.</div>';
  const cushion=Number(state.cushionMinimum)||0, vals=data.map(d=>d.value).concat([cushion]); const min=Math.min(...vals), max=Math.max(...vals); const span=Math.max(1,max-min); const W=640,H=250,pad=32;
  const xy=(v,i)=>({x:pad+(W-pad*2)*(data.length===1?.5:i/(data.length-1)),y:pad+(H-pad*2)*(1-(v-min)/span)});
  const pts=data.map((d,i)=>xy(d.value,i)); const poly=pts.map(p=>`${p.x},${p.y}`).join(' '); const cy=xy(cushion,0).y;
  const low=data.reduce((a,b)=>a.value<b.value?a:b), last=data.at(-1);
  return `<div class="forecast-graph-wrap"><svg class="forecast-graph" viewBox="0 0 ${W} ${H}" preserveAspectRatio="none"><line x1="${pad}" x2="${W-pad}" y1="${cy}" y2="${cy}" class="cushion-line"/><polyline points="${poly}" class="forecast-line"/>${pts.map((p,i)=>`<circle cx="${p.x}" cy="${p.y}" r="6" class="forecast-point"/><text x="${p.x}" y="${H-7}" text-anchor="middle" class="forecast-month-label">${monthName(data[i].m).slice(0,3)}</text>`).join('')}</svg><div class="graph-key"><span>🛡 Cuscinetto ${euroFmt.format(cushion)}</span><span>Minimo ${euroFmt.format(low.value)}</span><span>Fine ${euroFmt.format(last.value)}</span></div></div>`;
}
function forecasts(){const f=forecast(state.forecastMonth), cats=forecastExpenseCategories(state.forecastMonth);return `<section class="page ${state.page==='forecasts'?'active':''}"><div class="page-head"><div><h1>Previsioni</h1></div><button class="icon-btn" onclick="openSettings()">⚙</button></div><div class="month-control"><button onclick="shiftForecast(-1)">‹</button><select onchange="state.forecastMonth=this.value;save();render();requestAnimationFrame(()=>window.scrollTo(0,0))">${monthOptions().map(x=>`<option value="${x}" ${x===state.forecastMonth?'selected':''}>${monthName(x)}</option>`).join('')}</select><button onclick="shiftForecast(1)">›</button></div><div class="panel forecast-card"><h3 style="margin-top:0">Situazione prevista</h3><div class="forecast-row"><span>🟢 Entrate previste</span><b>${euroFmt.format(f.income)}</b></div><div class="forecast-row"><span>🔴 Uscite previste</span><b>${euroFmt.format(f.expenses)}</b></div><div class="forecast-row"><span>📉 Saldo mese</span><b class="${f.net>=0?'positive':'negative'}">${euroFmt.format(f.net)}</b></div><div class="forecast-row"><span>🏦 Totale stimato fine mese</span><b class="${f.endBalance>=0?'positive':'negative'}">${euroFmt.format(f.endBalance)}</b></div><div class="forecast-note">La previsione usa la <b>media di tutti i mesi precedenti disponibili</b>, categoria per categoria. Per il mese corrente considera anche i movimenti già registrati e stima solo il periodo rimanente; gli <b>eventi futuri</b> vengono aggiunti con il loro importo reale.</div></div><div class="panel forecast-categories"><h3>Ripartizione uscite previste</h3>${cats.map(([c,v])=>`<div class="forecast-row"><span>${c}</span><b class="negative">${euroFmt.format(v)}</b></div>`).join('')||'<div class="empty">Nessuna uscita prevista.</div>'}</div><div class="section-title">Eventi futuri <button class="primary-btn" onclick="openFutureEvent()">＋ Aggiungi evento</button></div><div class="events-list">${futureEventsForMonth(state.forecastMonth).map(eventRow).join('')||'<div class="empty">Nessun evento previsto per questo mese.</div>'}</div><div class="panel forecast-graph-panel"><h3>Andamento previsto dei risparmi</h3>${forecastGraphMarkup()}</div></section>`}
...
function goalRealAllocation(){
  const active=state.goals.filter(g=>Number(g.cost)>0).map(g=>({...g,realAllocated:0}));
...
function render(){
  ...
  try{
    ...
    initMovementSwipe();
    initGoalSwipe();
    requestAnimationFrame(()=>{ if(window.__mfForceTop){window.scrollTo(0,0);window.__mfForceTop=false;} });
  }catch(err){
...
function submitTransaction(ev){
  ...
  const transaction=existingId?{...state.transactions.find(t=>t.id===existingId),...data}:{id:uid(),...data};
