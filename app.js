...
const expenseCategories=['Obiettivi','Spesa','Benzina','Casa','Affitto','Contrattempo','Tempo libero','Altro'];
...
function currentDashboardWeekOffset(){
  ...
  const now=new Date();
  base.setHours(12,0,0,0); now.setHours(12,0,0,0);
  return Math.floor((now-base)/(7*24*60*60*1000));
}
...
return `<section class="page dashboard-page ${state.page==='dashboard'?'active':''}${dashboardWeekTransition?' week-changing':''}">
  <div class="hero">
    ...
    ${pickerControl({id:'dashboardMonthPicker',value:ym,label:monthName(ym),title:'Seleziona il mese',options:months.map(m=>({value:m,label:monthName(m)})),action:'dashboardMonth'})}
    <div class="balance-card"><div class="balance-label-row"><div class="balance-label">TOTALE DISPONIBILE</div></div><div class="balance-value-row"><div class="balance-value">${euroFmt.format(balanceAtEndOfMonth(ym))}</div><img class="balance-wallet" src="assets/wallet-transparent.png" alt="Portafoglio" /></div><div class="balance-delta"><span class="${sav>=0?'positive':'negative'}">${sav>=0?'↗':'↘'} ${euroFmt.format(sav)}</span> &nbsp; saldo del mese</div><div class="balance-buttons">${monthInsightButtons()}</div></div>
  </div>
...
function forecastChartMarkup(){
  ...
  const min=Math.min(...values,cushion), max=Math.max(...values,cushion,1), range=Math.max(1,max-min);
  const W=640,H=270,padL=82,padR=24,padT=30,padB=48;
  ...
  const grid=ticks.map(v=>`<g><line x1="${padL}" x2="${W-padR}" y1="${y(v).toFixed(1)}" y2="${y(v).toFixed(1)}" class="forecast-grid-line"/><text x="${padL-12}" y="${(y(v)+5).toFixed(1)}" text-anchor="end" class="forecast-axis-label">${euroFmt.format(v)}</text></g>`).join('');
  const labels=data.map((d,i)=>`<div class="forecast-chart-label" style="left:${(x(i)/W*100).toFixed(2)}%"><b>${monthName(d.m).slice(0,3)}</b><small>${euroFmt.format(d.value)}</small></div>`).join('');
  const startValue=values[0], endValue=values.at(-1), delta=endValue-startValue;
  return `<div class="forecast-chart-wrap"><div class="forecast-chart-explainer"><span>Saldo totale stimato alla fine di ogni mese</span><b class="${delta>=0?'positive':'negative'}">${delta>=0?'↗':'↘'} ${euroFmt.format(Math.abs(delta))} nei prossimi ${data.length-1} mesi</b></div><div class="forecast-chart"><svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="none" aria-label="Andamento del saldo totale previsto">${grid}<line x1="${padL}" x2="${W-padR}" y1="${cushionY}" y2="${cushionY}" class="cushion-line"/><text x="${W-padR}" y="${Math.max(padT+14,Number(cushionY)-8)}" text-anchor="end" class="cushion-label">Cuscinetto minimo</text><polyline points="${points}" class="forecast-line"/>${data.map((d,i)=>`<circle cx="${x(i).toFixed(1)}" cy="${y(d.value).toFixed(1)}" r="5" class="forecast-point"/>`).join('')}</svg><div class="forecast-chart-labels">${labels}</div></div></div>`;
}
function forecasts(){
...
function setDashboardMonth(m){state.dashboardMonth=m;save();render();requestAnimationFrame(()=>window.scrollTo({top:0,left:0,behavior:'auto'}))}
...
function goals(){const active=activeGoals(),completed=completedGoals();return `<section class="page ${state.page==='goals'?'active':''}"><div class="page-head"><div><h1>Obiettivi</h1></div><button class="icon-btn" onclick="openSettings()">⚙</button></div><button class="primary-btn" style="width:100%;font-size:18px" onclick="openGoal()">＋ Nuovo obiettivo</button><div class="goal-tabs"><button class="active">In corso (${active.length})</button><button id="completedGoalsTab">Completati (${completed.length})</button><button>Archiviati (0)</button></div><div class="goals-list">${active.map(goalCard).join('')||'<div class="empty">Non hai ancora obiettivi in corso.</div>'}</div></section>`}
...
function movements(){
  ...
  const items=state.transactions.map((t,index)=>({t,index})).filter(({t})=>(state.movementMonth==='all'||monthOf(t.date)===state.movementMonth)&&(state.movementFilter==='all'||t.type===state.movementFilter)).sort((a,b)=>{const byDate=b.t.date.localeCompare(a.t.date);if(byDate)return byDate;const byCreated=String(b.t.createdAt||'').localeCompare(String(a.t.createdAt||''));if(byCreated)return byCreated;return b.index-a.index;}).map(({t})=>t);
  return `<section class="page ${state.page==='movements'?'active':''}"><div class="page-head"><div><h1>Movimenti</h1></div><button class="primary-btn" onclick="openTransaction('income')">＋</button></div><div class="movement-controls"><div class="month-control movement-month-control">${pickerControl({id:'movementMonthPicker',value:state.movementMonth,label:state.movementMonth==='all'?'Tutti i mesi':monthName(state.movementMonth),title:'Mostra movimenti',options:[{value:'all',label:'Tutti i mesi'},...months.map(m=>({value:m,label:monthName(m)}))],action:'movementMonth'})}</div><div class="filters"><button class="filter-btn ${state.movementFilter==='all'?'active':''}" onclick="setFilter('all')">Tutti</button><button class="filter-btn ${state.movementFilter==='income'?'active':''}" onclick="setFilter('income')">Entrate</button><button class="filter-btn ${state.movementFilter==='expense'?'active':''}" onclick="setFilter('expense')">Uscite</button></div></div><div class="movement-list">${items.map(t=>{const isGoal=t.type==='expense'&&canonicalCategory(t.category)==='Obiettivi';const tone=isGoal?'goal':(t.type==='income'?'income':'expense');const icon=isGoal?goalMovementIcon(t):(t.type==='income'?'↗':'↘');return `<div class="movement-swipe-wrap" data-transaction-id="${t.id}"><div class="swipe-delete-action" onclick="requestDeleteTransaction('${t.id}')"><span>⌫</span><small>Elimina</small></div><div class="movement-row movement-${tone} swipeable-movement" onclick="openTransaction('${t.type}','','${t.id}')"><div class="event-icon">${icon}</div><div class="event-meta"><b class="${isGoal?'goal-movement-title':''}">${escapeHtml(t.description||t.category)}</b><small>${t.category} · ${new Date(t.date+'T12:00').toLocaleDateString('it-IT')}</small></div><div class="event-actions"><strong class="movement-amount ${isGoal?'goal-amount':(t.type==='income'?'positive':'negative')}">${t.type==='income'?'+':'-'}${euroFmt.format(t.amount)}</strong></div></div></div>`}).join('')||'<div class="empty">Nessun movimento.</div>'}</div></section>`
}
...
function pickerControl({id,name='',value='',label='',title='Seleziona',options=[],action='',date=false}){
  const encoded=encodeURIComponent(JSON.stringify(options));
  const safeValue=escapeHtml(value);
  const display=label||value;
  return `<input type="hidden" id="${id}" ${name?`name="${name}"`:''} value="${safeValue}"><button type="button" id="${id}Button" class="picker-control ${action==='dashboardMonth'?'month-select':''}" data-picker-target="${id}" data-picker-title="${escapeHtml(title)}" data-picker-action="${action}" data-picker-options="${encoded}" ${date?'data-picker-date="1"':''} onclick="${date?'openDatePickerFrom(this)':'openChoicePickerFrom(this)'}"><span>${escapeHtml(display)}</span><b>${date?'◷':'⌄'}</b></button>`;
}
let activePicker=null;
function openChoicePickerFrom(button){
  const options=JSON.parse(decodeURIComponent(button.dataset.pickerOptions||'[]'));
  activePicker={target:button.dataset.pickerTarget,title:button.dataset.pickerTitle,action:button.dataset.pickerAction,options};
  renderChoicePicker();
}
function renderChoicePicker(){
  if(!activePicker)return;
  const root=document.getElementById('dialogRoot');
  const target=document.getElementById(activePicker.target);
  const current=target?.value||'';
  root.innerHTML=`<div class="picker-backdrop" onclick="if(event.target===this)closePicker()"><div class="picker-sheet"><div class="picker-head"><div><small>MATTEO'S FINANCE</small><h2>${escapeHtml(activePicker.title)}</h2></div><button class="close-btn" onclick="closePicker()">×</button></div><div class="picker-options">${activePicker.options.map((o,i)=>{const value=typeof o==='string'?o:o.value;const label=typeof o==='string'?o:o.label;return `<button type="button" class="picker-option ${value===current?'selected':''}" onclick="choosePickerValue(${i})"><span>${escapeHtml(label)}</span><i>${value===current?'✓':''}</i></button>`}).join('')}</div></div></div>`;
}
function choosePickerValue(index){
  if(!activePicker)return;
  const option=activePicker.options[index];
  const value=typeof option==='string'?option:option.value;
  const label=typeof option==='string'?option:(option.label||option.value);
  const target=document.getElementById(activePicker.target),button=document.getElementById(activePicker.target+'Button');
  if(target)target.value=value;
  if(button)button.querySelector('span').textContent=label;
  const action=activePicker.action;
  closePicker();
  if(action==='dashboardMonth')setDashboardMonth(value);
  else if(action==='forecastMonth'){state.forecastMonth=value;save();render();window.scrollTo({top:0,behavior:'auto'});}
  else if(action==='movementMonth')setMovementMonth(value);
  else if(target){target.dispatchEvent(new Event('change',{bubbles:true}));}
}
function closePicker(){activePicker=null;document.getElementById('dialogRoot').innerHTML=''}
let activeDatePicker=null;
function openDatePickerFrom(button){
  const target=document.getElementById(button.dataset.pickerTarget);
  const value=target?.value||todayIso();
  activeDatePicker={target:button.dataset.pickerTarget,title:button.dataset.pickerTitle,value,cursor:monthOf(value)};
  renderDatePicker();
}
function renderDatePicker(){
  if(!activeDatePicker)return;
  const [year,month]=activeDatePicker.cursor.split('-').map(Number);
  const first=new Date(year,month-1,1),days=new Date(year,month,0).getDate(),start=(first.getDay()+6)%7;
  const cells=[];
  for(let i=0;i<start;i++)cells.push('<span class="date-empty"></span>');
  for(let d=1;d<=days;d++){
    const iso=`${year}-${pad2(month)}-${pad2(d)}`;
    cells.push(`<button type="button" class="date-day ${iso===activeDatePicker.value?'selected':''}" onclick="choosePickerDate('${iso}')">${d}</button>`);
  }
  const root=document.getElementById('dialogRoot');
  root.innerHTML=`<div class="picker-backdrop"><div class="date-picker"><div class="picker-head"><div><small>MATTEO'S FINANCE</small><h2>${escapeHtml(activeDatePicker.title)}</h2></div><button class="close-btn" onclick="closePicker()">×</button></div><div class="date-picker-selected">${new Date(activeDatePicker.value+'T12:00').toLocaleDateString('it-IT',{day:'numeric',month:'long',year:'numeric'})}</div><div class="date-picker-nav"><button type="button" onclick="shiftPickerMonth(-1)">‹</button><b>${monthName(activeDatePicker.cursor)}</b><button type="button" onclick="shiftPickerMonth(1)">›</button></div><div class="date-weekdays"><span>L</span><span>M</span><span>M</span><span>G</span><span>V</span><span>S</span><span>D</span></div><div class="date-grid">${cells.join('')}</div><div class="date-picker-actions"><button class="secondary-btn" type="button" onclick="closePicker()">Annulla</button><button class="primary-btn" type="button" onclick="choosePickerDate(activeDatePicker.value)">Conferma</button></div></div></div>`;
}
function shiftPickerMonth(delta){if(!activeDatePicker)return;activeDatePicker.cursor=delta>0?nextMonth(activeDatePicker.cursor):prevMonth(activeDatePicker.cursor);renderDatePicker()}
function prevMonth(ym){const [y,m]=ym.split('-').map(Number);return m===1?`${y-1}-12`:`${y}-${pad2(m-1)}`}
function choosePickerDate(value){
  if(!activeDatePicker)return;
  activeDatePicker.value=value;
  const targetId=activeDatePicker.target;
  const target=document.getElementById(targetId),button=document.getElementById(targetId+'Button');
  if(target)target.value=value;
  if(button)button.querySelector('span').textContent=new Date(value+'T12:00').toLocaleDateString('it-IT');
  closePicker();
  if(target)target.dispatchEvent(new Event('change',{bubbles:true}));
}
function setPickerOptions(targetId,options,value){
  const button=document.getElementById(targetId+'Button'),target=document.getElementById(targetId);if(!button||!target)return;
  const normalized=options.map(o=>typeof o==='string'?{value:o,label:o}:o);
  const nextValue=normalized.some(o=>o.value===value)?value:(normalized[0]?.value||'');
  button.dataset.pickerOptions=encodeURIComponent(JSON.stringify(normalized));
  target.value=nextValue;
  button.querySelector('span').textContent=(normalized.find(o=>o.value===nextValue)?.label)||nextValue;
}
function showAppAlert(message,title='Attenzione'){
  const root=document.getElementById('dialogRoot');
  root.innerHTML=`<div class="app-dialog-backdrop"><div class="app-dialog"><div class="app-dialog-glow"></div><div class="app-dialog-icon">◆</div><h2>${escapeHtml(title)}</h2><p>${escapeHtml(message).replace(/\n/g,'<br>')}</p><div class="app-dialog-actions"><button class="primary-btn" onclick="closeAppDialog()">OK</button></div></div></div>`;
}
function showAppConfirm(message,onConfirm,{title='Conferma operazione',danger=false,confirmText='Conferma'}={}){
  const root=document.getElementById('dialogRoot');
  const key='dialogConfirm_'+uid();
  window[key]=()=>{closeAppDialog();delete window[key];onConfirm&&onConfirm()};
  root.innerHTML=`<div class="app-dialog-backdrop"><div class="app-dialog"><div class="app-dialog-glow"></div><div class="app-dialog-icon ${danger?'danger':''}">${danger?'!':'?'}</div><h2>${escapeHtml(title)}</h2><p>${escapeHtml(message).replace(/\n/g,'<br>')}</p><div class="app-dialog-actions"><button class="secondary-btn" onclick="closeAppDialog()">Annulla</button><button class="${danger?'danger-btn':'primary-btn'}" onclick="window['${key}']()">${escapeHtml(confirmText)}</button></div></div></div>`;
}
function closeAppDialog(){document.getElementById('dialogRoot').innerHTML=''}
...
function selectType(type){const f=document.getElementById('transactionForm');f.dataset.type=type;document.getElementById('typeIncome').classList.toggle('active',type==='income');document.getElementById('typeExpense').classList.toggle('active',type==='expense');const category=document.getElementById('transactionCategory');if(category){setPickerOptions('transactionCategory',categoriesForType(type),categoriesForType(type)[0]);syncTransactionDescriptionRequirement();}}
function submitTransaction(ev){
  ...
  if(canonicalCategory(data.category)==='Obiettivi'&&!description){showAppAlert('Per un movimento Obiettivi la descrizione è obbligatoria.','Descrizione richiesta');return;}
  const existingId=f.dataset.editId;
  const previous=existingId?state.transactions.find(t=>t.id===existingId):null;
  const transaction=existingId?{...previous,...data}:{id:uid(),createdAt:new Date().toISOString(),...data};
  ...
  if(match){
    ...
    showAppConfirm(`Possibile evento futuro trovato.
...
Era per caso l'evento futuro che stavamo aspettando?`,()=>{state.futureEvents=state.futureEvents.filter(e=>e.id!==event.id);recalculateBalance();save();render();},{title:'Evento futuro',confirmText:'Sì, conferma'});
  }
...
function finalizeGoalVictory(goalId){
  ...
  const complete=()=>{goal.completed=true;goal.completionPending=false;goal.completedAt=todayIso();save();render();setTimeout(()=>{const tab=document.getElementById('completedGoalsTab');if(tab){tab.classList.add('completed-pulse');setTimeout(()=>tab.classList.remove('completed-pulse'),4200)}},80)};
...
function importBackup(event){
  ...
  reader.onload=()=>{
    try{
      ...
      showAppConfirm('Importando il backup i dati attuali verranno sostituiti. Vuoi continuare?',()=>{
      const safe={
        ...structuredClone(DEFAULT_STATE),
        ...imported,
      ...
      closeModal();
      render();
      showAppAlert('Backup importato correttamente.','Backup importato');
      },{title:'Importa backup',danger:true,confirmText:'Importa'});
    }catch(err){
      showAppAlert('Impossibile importare il backup: il file non è valido.','Importazione non riuscita');
...
function openDeleteTransactions(){const months=[...new Set(state.transactions.map(t=>monthOf(t.date)))].sort();openModal(`<div class="modal-head"><h2>Cancella entrate/uscite</h2><button class="close-btn" onclick="closeModal()">×</button></div><div class="form-grid"><div class="field"><label>Seleziona il mese da cancellare</label>${pickerControl({id:'deleteMonth',value:'',label:'— Seleziona un mese —',title:'Seleziona il mese',options:[{value:'',label:'— Seleziona un mese —'},...months.map(m=>({value:m,label:monthName(m)}))]})}</div><button class="secondary-btn" onclick="confirmDeleteMonth()">Cancella il mese selezionato</button><button class="danger-btn" onclick="confirmDeleteAllTransactions()">Cancella tutti i mesi</button><button class="secondary-btn" onclick="openSettings()">Annulla</button></div>`)}
function confirmDeleteMonth(){const m=document.getElementById('deleteMonth').value;if(!m)return;showAppConfirm('Sicuro che vuoi cancellare tutti i dati di '+monthName(m)+'?',()=>{const removed=state.transactions.filter(t=>monthOf(t.date)===m);state.transactions=state.transactions.filter(t=>monthOf(t.date)!==m);restoreGoalsLinkedToTransactions(removed);recalculateBalance();save();closeModal();render()},{title:'Cancella mese',danger:true,confirmText:'Cancella'})}
function confirmDeleteAllTransactions(){showAppConfirm('Sicuro che vuoi cancellare tutte le entrate e le uscite?',()=>{const removed=[...state.transactions];state.transactions=[];restoreGoalsLinkedToTransactions(removed);recalculateBalance();save();closeModal();render()},{title:'Cancella tutti i movimenti',danger:true,confirmText:'Cancella tutto'})}
function confirmResetMonth(){const m=currentMonth();showAppConfirm('Vuoi davvero cancellare i dati del mese corrente?',()=>{const removed=state.transactions.filter(t=>monthOf(t.date)===m);state.transactions=state.transactions.filter(t=>monthOf(t.date)!==m);restoreGoalsLinkedToTransactions(removed);state.futureEvents=state.futureEvents.filter(e=>monthOf(e.date)!==m);recalculateBalance();save();closeModal();render()},{title:'Reset mese',danger:true,confirmText:'Resetta'})}
function confirmResetTotal(){showAppConfirm('Vuoi davvero cancellare tutti i dati? Questa operazione non si può annullare.',()=>{state.balance=0;state.startingCapital=0;state.startingCapitalMonth='';state.transactions=[];state.futureEvents=[];state.goals=[];state.cushionMinimum=0;state.cushionIdeal=0;save();closeModal();render()},{title:'Reset totale',danger:true,confirmText:'Resetta tutto'})}
...
function resetDemo(){showAppConfirm('Ripristinare i dati iniziali?',()=>{localStorage.removeItem('matteos-finance-v1');location.reload()},{title:'Ripristina dati',danger:true,confirmText:'Ripristina'})}
