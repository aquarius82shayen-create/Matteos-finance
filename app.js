...
function futureEventsConsultationMarkup(){
  const rows=futureEventsConsultation().map(e=>`<div class="future-events-consult-row"><span>${escapeHtml(new Date(e.date+'T12:00:00').toLocaleDateString('it-IT',{day:'numeric',month:'long',year:'numeric'}))} - ${escapeHtml(e.description||'Evento')} - ${euroFmt.format(Number(e.amount)||0)}</span></div>`).join('');
  return `<div class="panel future-events-consult-panel"><h3>Tutti gli eventi futuri</h3><div class="future-events-consult-list">${rows||'<div class="empty">Nessun evento futuro programmato.</div>'}</div></div>`;
}
