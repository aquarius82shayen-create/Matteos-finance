...
function bestWorstMonthCards(){
  ...
  const card=(kind,item,label)=>`<button class="month-insight-card ${kind}" onclick="openMonthAnalysis('${item.m}')"><small>${label}</small><strong>${monthName(item.m)}</strong><b>${item.net>=0?'+':''}${euroFmt.format(item.net)}</b></button>`;
