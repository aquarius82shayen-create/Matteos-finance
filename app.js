...
function save(){invalidatePerformanceCache();localStorage.setItem('matteos-finance-v1',JSON.stringify(state))}
...
function movements(){
  ...
  const items=state.transactions.filter(t=>(state.movementMonth==='all'||monthOf(t.date)===state.movementMonth)&&(state.movementFilter==='all'||t.type===state.movementFilter)).slice().sort((a,b)=>b.date.localeCompare(a.date));
