...
function goalCard(g){const a=analyseGoal(g),real=goalRealAllocation()[g.id]||0,pct=Math.min(100,real/g.cost*100);return `<div class="goal-swipe-wrap" data-goal-id="${g.id}"><div class="swipe-delete-action" onclick="requestDeleteGoal('${g.id}')"><span>⌫</span><small>Elimina</small></div><div class="goal-card swipeable-goal"><div class="goal-content"><div class="goal-top"><div><h3 class="editable-title" onclick="openGoal('${g.id}')">${escapeHtml(g.name)}</h3><b class="positive goal-cost">${euroFmt.format(g.cost)}</b><p>Entro il ${new Date(g.deadline+'T12:00').toLocaleDateString('it-IT')}</p><p>Priorità ${g.priority}</p></div><div class="goal-side"><span class="status ${a.status}">${a.label}</span><div class="goal-visual" aria-hidden="true">${goalVisual(g.name)}</div></div></div><div class="progress"><i style="width:${pct}%"></i></div><div class="goal-foot"><span>${euroFmt.format(real)}</span><span>${Math.round(pct)}%</span><span>${euroFmt.format(g.cost)}</span></div><div class="goal-actions"><button class="secondary-btn" style="flex:1" onclick="openGoalAnalysis('${g.id}')">Analizza</button></div></div></div></div>`}
function analyseGoal(g){
  const reference=currentMonth(), months=goalMonthsLeft(g,reference);
  const real=goalRealAllocation()[g.id]||0;
  const needed=months>0?Math.max(0,(g.cost-real)/months):Math.max(0,g.cost-real);
  const plan=goalPlan(), p=plan.plans[g.id]||{};
  const capacity=months>0?sum(Object.entries(p.monthly||{}).filter(([ym])=>ym<=goalDeadlineMonth(g)).map(([,v])=>Number(v)||0))/months:0;
  ...
  return {monthsLeft:months,needed,capacity,status,label,completionMonth,completionDate,deadlineReachable,monthDifference,real,cushionMinimum:plan.cushionMinimum,cushionIdeal:plan.cushionIdeal}
}
function movements(){
...
function openGoalAnalysis(id){const g=state.goals.find(x=>x.id===id);if(!g)return;const a=analyseGoal(g);const priority=g.priority.charAt(0).toUpperCase()+g.priority.slice(1);let result='';if(a.completionDate){if(a.monthDifference!==null&&a.monthDifference<0)result=`Il tuo obiettivo può essere realizzato in ${a.completionDate}, circa ${Math.abs(a.monthDifference)} ${Math.abs(a.monthDifference)===1?'mese':'mesi'} prima della scadenza.`;else if(a.monthDifference===0)result=`Il tuo obiettivo è previsto in ${a.completionDate}, in linea con la scadenza.`;else result=`Il tuo obiettivo può essere realizzato in ${a.completionDate}, circa ${a.monthDifference} ${a.monthDifference===1?'mese':'mesi'} dopo la scadenza.`}else result='Con le previsioni attuali e le priorità impostate non risultano risorse sufficienti per stimare una data di realizzazione.';openModal(`<div class="modal-head"><h2>Analisi obiettivo</h2><button class="close-btn" onclick="closeModal()">×</button></div><div class="analysis-box ${a.status}" style="margin-top:16px"><h3 style="margin-top:0">${escapeHtml(g.name)} <b class="goal-analysis-cost">${euroFmt.format(g.cost)}</b></h3><div class="forecast-row"><span>Priorità</span><b>${priority}</b></div><div class="forecast-row"><span>Scadenza</span><b>${new Date(g.deadline+'T12:00').toLocaleDateString('it-IT')}</b></div><div class="forecast-row"><span>Risparmio mensile necessario</span><b>${euroFmt.format(a.needed)}</b></div><div class="forecast-row"><span>Quota media prevista</span><b>${euroFmt.format(a.capacity)}</b></div><h3>${a.label}</h3><p>${result}</p><p><small>La barra dell'obiettivo mostra soltanto il progresso reale e può avanzare o retrocedere in base al saldo effettivamente disponibile dopo il cuscinetto. Nei mesi senza entrate non viene assegnata alcuna quota agli obiettivi.</small></p></div><div class="modal-actions"><button class="primary-btn" onclick="closeModal();render()">Chiudi</button></div>`)}
