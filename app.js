...
function monthInsightButtons(){
  ...
  if(!months.length){
    return `<button class="big-btn income" disabled>Mese migliore<br><small>In attesa di dati</small></button><button class="big-btn expense" disabled>Mese più difficile<br><small>In attesa di dati</small></button>`;
  }
  ...
  return `<button class="big-btn income" onclick="openMonthAnalysis('${best.m}')">Mese migliore<br><small>${monthName(best.m)} · ${best.net>=0?'+':''}${euroFmt.format(best.net)}</small></button><button class="big-btn expense" onclick="openMonthAnalysis('${worst.m}')">Mese più difficile<br><small>${monthName(worst.m)} · ${worst.net>=0?'+':''}${euroFmt.format(worst.net)}</small></button>`;
}
...
function openMonthAnalysis(ym){
  ...
  const avgNet=previous.length?sum(previous.map(m=>income(m)-expenses(m)))/previous.length:null;
  const topExpense=expenseCats[0]; const topIncome=incomeCats[0];
  const unusual=avgNet!==null&&Math.abs(net-avgNet)>Math.max(100,Math.abs(avgNet)*.35);
  let lines=[];
  if(net>=0){
    lines.push(`Mese chiuso con un saldo positivo di <b>${euroFmt.format(net)}</b>.`);
    if(topIncome) lines.push(`La voce di entrata più importante è stata <b>${escapeHtml(topIncome[0])}</b>.`);
    if(topExpense) lines.push(`La spesa che ha inciso di più è stata <b>${escapeHtml(topExpense[0])}</b>.`);
    lines.push(unusual?'Il risultato si discosta sensibilmente dalla media recente: vale la pena capire cosa è stato eccezionale e cosa puoi replicare.':'Il mese è stato finanziariamente equilibrato rispetto ai dati disponibili.');
    lines.push('Consiglio: conserva le abitudini che hanno funzionato e considera il surplus come margine di sicurezza, non come spesa già disponibile.');
  }else{
    lines.push(`Mese chiuso con un saldo negativo di <b>${euroFmt.format(net)}</b>.`);
    if(topExpense) lines.push(`La categoria che ha pesato maggiormente è stata <b>${escapeHtml(topExpense[0])}</b>.`);
    lines.push(unusual?'Il risultato è stato più anomalo del solito: potrebbe dipendere da una spesa straordinaria o da un evento non ricorrente, non necessariamente da una cattiva gestione.':'Le uscite hanno superato le entrate: il dato merita attenzione, ma è un segnale utile per correggere la rotta.');
    lines.push('Consiglio: controlla quali spese erano inevitabili e quali categorie possono essere ridotte nel mese successivo senza sacrificare ciò che conta davvero.');
  }
