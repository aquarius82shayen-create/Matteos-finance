...
function openFinanceDate(input){if(!input||input.disabled)return;const root=pickerRoot();const dateName=input.dataset.dateName||input.name;const hidden=input.form?.querySelector(`input[type="hidden"][name="${dateName}"]`)||input.parentElement?.querySelector(`input[type="hidden"][name="${dateName}"]`);let selected=hidden?.value||input.dataset.dateValue||input.value||'';let initial=selected?new Date(selected+'T12:00:00'):new Date();if(dateName==='deadline'){const start= input.form?.querySelector('input[type="hidden"][name="startDate"]')?.value;if(start)initial=new Date(start+'T12:00:00')}let viewYear=initial.getFullYear(),viewMonth=initial.getMonth();const renderCalendar=()=>{const days=new Date(Date.UTC(viewYear,viewMonth+1,0)).getUTCDate(),start=(new Date(Date.UTC(viewYear,viewMonth,1)).getUTCDay()+6)%7;let cells='';for(let i=0;i<start;i++)cells+='<span class="finance-day empty"></span>';for(let day=1;day<=days;day++){const iso=`${viewYear}-${pad2(viewMonth+1)}-${pad2(day)}`;cells+=`<button type="button" class="finance-day ${iso===selected?'selected':''} ${iso===todayIso()?'today':''}" data-finance-date="${iso}">${day}</button>`}root.innerHTML=`<div class="finance-picker-backdrop" onclick="if(event.target===this)closeFinancePicker()"><div class="finance-picker-card finance-date-picker"><button type="button" class="finance-picker-close" onclick="closeFinancePicker()">×</button><div class="finance-calendar-nav"><button type="button" data-finance-prev>‹</button><b>${monthTitle(viewYear,viewMonth)}</b><button type="button" data-finance-next>›</button></div><div class="finance-weekdays"><span>L</span><span>M</span><span>M</span><span>G</span><span>V</span><span>S</span><span>D</span></div><div class="finance-calendar-grid">${cells}</div></div></div>`;root.querySelector('[data-finance-prev]').onclick=()=>{viewMonth--;if(viewMonth<0){viewMonth=11;viewYear--}renderCalendar()};root.querySelector('[data-finance-next]').onclick=()=>{viewMonth++;if(viewMonth>11){viewMonth=0;viewYear++}renderCalendar()};root.querySelectorAll('[data-finance-date]').forEach(btn=>btn.onclick=()=>{const iso=btn.dataset.financeDate;selected=iso;if(hidden)hidden.value=iso;input.dataset.dateValue=iso;input.value=displayDate(iso);input.dispatchEvent(new Event('input',{bubbles:true}));input.dispatchEvent(new Event('change',{bubbles:true}));closeFinancePicker()})};renderCalendar()}
