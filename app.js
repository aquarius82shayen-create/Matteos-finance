...
function daysUntilMonthInsights(){
  const now=new Date();
  now.setHours(0,0,0,0);
  const allMonths=[...new Set(state.transactions.map(t=>monthOf(t.date)))].sort();
  const firstMonth=allMonths[0]||currentMonth();
  const [year,month]=firstMonth.split('-').map(Number);
  // Servono due mesi di calendario conclusi: se si parte a settembre,
  // settembre è disponibile dal 1° ottobre e il secondo mese dal 1° novembre.
  const availableFrom=new Date(year,month+1,1);
  return Math.max(0,Math.ceil((availableFrom-now)/86400000));
}
function monthInsightButtons(){
  const months=completedInsightMonths();
  if(months.length<2){
    const days=daysUntilMonthInsights();
    return `<button class="big-btn income month-insight-btn" disabled>Disponibile tra</button><button class="big-btn expense month-insight-btn" disabled>${days} giorni</button>`;
  }
  ...
  return `<button class="big-btn income month-insight-btn" onclick="openMonthAnalysis('${best.m}')">Mese migliore</button><button class="big-btn expense month-insight-btn" onclick="openMonthAnalysis('${worst.m}')">Mese peggiore</button>`;
}
...
function openMonthAnalysis(ym){
  ...
  const previousNets=previous.map(m=>income(m)-expenses(m));
  const avgNet=previousNets.length?sum(previousNets)/previousNets.length:null;
  const avgInc=previous.length?sum(previous.map(income))/previous.length:null;
  const avgOut=previous.length?sum(previous.map(expenses))/previous.length:null;
  const topExpense=expenseCats[0], topIncome=incomeCats[0];
  const previousExpenseByCat={};
  previous.forEach(m=>monthCategoryTotals(m,'expense').forEach(([c,v])=>{(previousExpenseByCat[c]??=[]).push(v)}));
  const previousIncomeByCat={};
  previous.forEach(m=>monthCategoryTotals(m,'income').forEach(([c,v])=>{(previousIncomeByCat[c]??=[]).push(v)}));
  const expenseAnomalies=expenseCats.map(([c,v])=>{
    const vals=previousExpenseByCat[c]||[]; const avg=vals.length?sum(vals)/vals.length:0;
    return {c,v,avg,diff:v-avg,ratio:avg? v/avg : (v?Infinity:1)};
  }).filter(x=>x.diff>Math.max(50,x.avg*.35)).sort((a,b)=>b.diff-a.diff);
  const incomeAnomalies=incomeCats.map(([c,v])=>{
    const vals=previousIncomeByCat[c]||[]; const avg=vals.length?sum(vals)/vals.length:0;
    return {c,v,avg,diff:v-avg};
  }).filter(x=>x.diff>Math.max(50,x.avg*.25)).sort((a,b)=>b.diff-a.diff);
  const unusual=avgNet!==null&&Math.abs(net-avgNet)>Math.max(100,Math.abs(avgNet)*.35);
  const lines=[];
  if(net>=0){
    lines.push(`Hai chiuso il mese con un saldo positivo di <b>${euroFmt.format(net)}</b>${avgNet!==null?` (${net>=avgNet?'sopra':'sotto'} la tua media recente di ${euroFmt.format(avgNet)})`:''}.`);
    if(avgInc!==null&&avgOut!==null){
      if(inc>avgInc*1.12) lines.push(`Il risultato è stato aiutato da entrate superiori al solito, quindi valuta quanto di questo miglioramento sia realmente replicabile.`);
      else if(out<avgOut*.88) lines.push(`Hai contenuto le uscite rispetto alla tua media recente: questo è il comportamento più utile da provare a mantenere.`);
    }
    if(incomeAnomalies[0]) lines.push(`La voce <b>${escapeHtml(incomeAnomalies[0].c)}</b> ha contribuito più del normale al risultato.`);
    if(topExpense && expenseAnomalies[0]) lines.push(`Attenzione però: <b>${escapeHtml(expenseAnomalies[0].c)}</b> è cresciuta rispetto alla sua media e merita un controllo.`);
    else if(topExpense) lines.push(`La spesa principale è stata <b>${escapeHtml(topExpense[0])}</b>, senza anomalie particolarmente critiche nei dati disponibili.`);
    lines.push(`Consiglio: non considerare tutto il surplus come già spendibile; prima verifica cosa è necessario per il mese successivo e per il cuscinetto.`);
  }else{
    lines.push(`Hai chiuso il mese con un saldo negativo di <b>${euroFmt.format(net)}</b>${avgNet!==null?` (${net<avgNet?'peggiore':'migliore'} della tua media recente di ${euroFmt.format(avgNet)})`:''}.`);
    if(expenseAnomalies[0]) lines.push(`La causa principale sembra essere <b>${escapeHtml(expenseAnomalies[0].c)}</b>, salita di circa <b>${euroFmt.format(expenseAnomalies[0].diff)}</b> rispetto alla media osservata.`);
    else if(avgInc!==null&&inc<avgInc*.88) lines.push(`Le entrate sono risultate più basse del solito, quindi il mese negativo non dipende necessariamente da un eccesso di spese.`);
    else if(topExpense) lines.push(`La categoria che ha inciso maggiormente è stata <b>${escapeHtml(topExpense[0])}</b>; è il primo punto da controllare per capire se la spesa era inevitabile o riducibile.`);
    if(unusual) lines.push(`Il risultato è abbastanza distante dal tuo andamento recente: trattalo come un segnale da analizzare, non come una bocciatura della gestione.`);
    lines.push(`Consiglio: separa le spese straordinarie da quelle ricorrenti. Se il problema è occasionale, il prossimo mese può recuperare; se è ricorrente, conviene intervenire sulla categoria più pesante.`);
  }
...
function forecastChartMarkup(){
  ...
  return `<div class="forecast-chart-wrap"><div class="forecast-chart-summary"><span>Minimo <b>${euroFmt.format(Math.min(...values))}</b></span><span>Finale <b>${euroFmt.format(values.at(-1))}</b></span><span>🛡 Cuscinetto <b>${euroFmt.format(cushion)}</b></span></div><div class="forecast-chart"><svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="none" aria-label="Andamento del saldo previsto"><line x1="${padX}" x2="${W-padX}" y1="${cushionY}" y2="${cushionY}" class="cushion-line"/><polyline points="${points}" class="forecast-line"/>${data.map((d,i)=>`<circle cx="${x(i).toFixed(1)}" cy="${y(d.value).toFixed(1)}" r="5" class="forecast-point"><title>${monthName(d.m)}: ${euroFmt.format(d.value)}</title></circle>`).join('')}</svg><div class="forecast-chart-labels">${labels}</div></div></div>`;
}
