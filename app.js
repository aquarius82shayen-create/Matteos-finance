...
const ICONS={
  ...
  stadio:'138_stadio.webp', sport:'139_sport.webp', museo:'140_museo.webp', pallavolo:'141_pallavolo.webp', calcio:'142_calcio.webp', basket:'143_basket.webp', surf:'144_surf.webp', bilancia:'100_bilancia_antica.webp', rugby:'146_rugby.webp', libri:'147_libri.webp', musica:'148_musica.webp', aperitivoNamed:'149_aperitivo.webp', pizza:'150_pizza.webp', cucinaCinese:'151_cucina_cinese.webp', hamburger:'152_hamburger.webp', irlanda:'153_irlanda.webp', uk:'68_thailandia.webp'
};
...
function semanticIconFile(description='',category='',type=''){
  ...
  const hasWord=(word)=>{
    ...
    return new RegExp(`(^|\\s)${w.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}(?=\\s|$)`,'u').test(text);
  };
  const hasAny=(words)=>words.some(hasWord);
  const hasPhrase=(phrases)=>phrases.some(p=>text.includes(
    String(p).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'')
  ));

  // Lightweight semantic scoring: description only. Specific concepts receive
  // more weight than places or broad themes. No network lookup is performed.
  const scores=new Map();
  const add=(key,weight)=>scores.set(key,(scores.get(key)||0)+weight);
  const rule=(key,words,weight=8,phrases=[])=>{
    if(hasAny(words))add(key,weight);
    if(phrases.length&&hasPhrase(phrases))add(key,weight+5);
  };

  // Strong compound concepts: different wording can express the same intent.
  if((hasAny(['bilanciare','bilancia','pareggiare','pareggia','equilibrare','equilibrio']) &&
      hasAny(['entrate','entrata']) &&
      hasAny(['uscite','uscita','spese','spesa','bilancio']))) add('bilancia',30);
  if((hasAny(['concerto','concerti','concerto'])||hasAny(['live'])) &&
      hasAny(['iron','metal','musica','musicale'])) add('concert',28);
  if(hasAny(['vacanza','vacanze','ferie','weekend']) && hasAny(['valledoria'])) add('travel',24);

  // Specific objects / activities.
  rule('light',['lampadina','lampade','lampada','luce','luci'],18);
  rule('bilancia',['bilanciare','bilancia','pareggiare','equilibrare','bilancio'],10);
  rule('concert',['concerto','concerti','live','spettacolo musicale'],18);
  rule('musica',['iron','maiden','metal','heavy metal','musica','musicale'],15);
  rule('fuel',['benzina','carburante','rifornimento','rifornire','diesel'],18);
  rule('flight',['aereo','aeroporto','aeroporto','volo','volare','voli'],17);
  rule('train',['treno','treni','ferrovia','ferroviario'],17);
  rule('cruise',['crociera','crociere'],17);
  rule('hotel',['hotel','albergo','alloggio','b&b','bed','pensione'],17);
  rule('camping',['campeggio','campeggiare','tenda'],17);
  rule('gym',['palestra','allenamento','fitness','gym'],17);
  rule('doctor',['medico','dottore','visita medica','specialista'],17);
  rule('dentist',['dentista','dentale'],17);
  rule('pharmacy',['farmacia','farmaci','farmaco','medicinale','medicinali'],17);
  rule('vet',['veterinario','veterinaria'],17);
  rule('restaurant',['ristorante','ristorazione'],16);
  rule('dinner',['cena','cenare','pranzo','pranzare'],16);
  rule('aperitivo',['aperitivo','aperitivi','cocktail'],16);
  rule('pizza',['pizza','pizze'],18);
  rule('hamburger',['hamburger','burger'],18);
  rule('wedding',['matrimonio','matrimoni','nozze'],18);
  rule('christmas',['natale','christmas'],18);
  rule('newYear',['capodanno','anno nuovo'],18);
  rule('easter',['pasqua'],18);
  rule('ferragosto',['ferragosto'],18);
  rule('party',['compleanno','compleanni','festa','feste','party'],14);
  rule('gift',['regalo','regali','omaggio'],16);
  rule('charity',['donazione','donazioni','beneficenza'],16);
  rule('taxes',['tasse','imposte','f24','tributi'],16);
  rule('multa',['multa','multe','sanzione','sanzioni'],16);
  rule('creditCard',['carta di credito','carta credito','credit card'],16);
  rule('carTax',['bollo auto','bollo macchina'],18);
  rule('carInsurance',['assicurazione auto','assicurazione macchina'],18);
  rule('repair',['meccanico','riparazione auto','gommista','officina'],16);
  rule('driving',['guidare','guida','automobile','macchina','auto'],9);
  rule('house',['casa','abitazione','appartamento'],9);
  rule('appliances',['elettrodomestico','elettrodomestici','frigorifero','lavatrice','forno'],15);
  rule('gas',['gas','metano'],15);
  rule('internet',['internet','wifi','wi fi','fibra','adsl'],15);
  rule('bills',['bolletta','bollette','utenze','utenza'],15);
  rule('moving',['trasloco','traslocare'],15);
  rule('tools',['attrezzi','utensili','ferramenta'],15);
  rule('love',['amore','fidanzata','fidanzato','coppia','anniversario'],15);
  rule('health',['salute','benessere'],13);
  rule('family',['famiglia','figlio','figlia','genitori'],14);
  rule('mushrooms',['funghi','fungo'],18);
  rule('pet',['animale','animali','gatto','gatti','cane','cani'],15);
  rule('sunglasses',['occhiali da sole','occhiali'],16);
  rule('shopping',['shopping','acquisti','acquisto','negozio','shopping'],12);
  rule('electronics',['elettronica','computer','pc','telefono','smartphone','tablet'],15);
  rule('target',['obiettivo','obiettivi','traguardo'],16);
  rule('wallet',['portafoglio','soldi','denaro','contanti'],12);
  rule('party',['serata','serata fuori'],7);

  // Travel / geographic concepts are deliberately weaker than explicit
  // activities/objects, so "concerto ... a Dublino" stays Concerto.
  rule('travel',['viaggio','viaggi','vacanza','vacanze','ferie','weekend','tour'],11);
  rule('beach',['spiaggia','mare','balneare','bagno'],12);
  rule('mountain',['montagna','montagne','trekking','escursione','escursioni'],12);
  rule('bar',['bar','pub'],11);
  rule('theatre',['teatro'],14);

  // Countries / places.
  const places=[
    ['irlanda',['dublino','irlanda']],
    ['uk',['londra','inghilterra','regno unito','uk','united kingdom']],
    ['germania',['berlino','germania']],
    ['italia',['roma','italia','sardegna','valledoria']],
    ['spagna',['madrid','spagna']],
    ['francia',['parigi','francia']],
    ['portogallo',['lisbona','portogallo']],
    ['austria',['vienna','austria']],
    ['grecia',['atene','grecia']],
    ['turchia',['istanbul','turchia']],
    ['croazia',['zagabria','croazia']],
    ['repubblicaCeca',['praga','repubblica ceca']],
    ['ungheria',['budapest','ungheria']],
    ['polonia',['varsavia','polonia']],
    ['slovacchia',['bratislava','slovacchia']],
    ['slovenia',['lubiana','slovenia']],
    ['islanda',['reykjavik','islanda']],
    ['norvegia',['oslo','norvegia']],
    ['svezia',['stoccolma','svezia']],
    ['danimarca',['copenaghen','danimarca']],
    ['finlandia',['helsinki','finlandia']],
    ['lituania',['vilnius','lituania']],
    ['lettonia',['riga','lettonia']],
    ['estonia',['tallinn','estonia']],
    ['belgio',['bruxelles','belgio']],
    ['romania',['bucarest','bucharest','romania']],
    ['bulgaria',['sofia','bulgaria']],
    ['malta',['malta']],
    ['messico',['messico']],
    ['usa',['stati uniti','usa','new york','america']],
    ['giappone',['giappone','tokyo','kyoto']],
    ['cina',['cina','pechino','shanghai']],
    ['canada',['canada','toronto','vancouver']],
    ['thailandia',['thailandia','bangkok']],
    ['vietnam',['vietnam']],
  ...
  places.forEach(([key,words])=>{if(hasAny(words))add(key,5)});

  // Map semantic keys to the existing icon library.
  const map={
    light:ICONS.light,bilancia:ICONS.bilancia,concert:ICONS.concert,musica:ICONS.musica,
    fuel:ICONS.fuel,flight:ICONS.flight,train:ICONS.train,cruise:ICONS.cruise,hotel:ICONS.hotel,
    camping:ICONS.camping,gym:ICONS.gym,doctor:ICONS.doctor,dentist:ICONS.dentist,
    pharmacy:ICONS.pharmacy,vet:ICONS.vet,restaurant:ICONS.restaurant,dinner:ICONS.dinner,
    aperitivo:ICONS.aperitivo,pizza:ICONS.pizza,hamburger:ICONS.hamburger,wedding:ICONS.wedding,
    christmas:ICONS.christmas,newYear:ICONS.newYear,easter:ICONS.easter,ferragosto:ICONS.ferragosto,
    party:ICONS.party,gift:ICONS.gift,charity:ICONS.charity,taxes:ICONS.taxes,
    multa:ICONS.multa,creditCard:ICONS.creditCard,carTax:ICONS.carTax,carInsurance:ICONS.carInsurance,
    repair:ICONS.repair,driving:ICONS.driving,house:ICONS.house,appliances:ICONS.appliances,
    gas:ICONS.gas, internet:ICONS.internet,bills:ICONS.bills,moving:ICONS.moving,tools:ICONS.tools,
    love:ICONS.love,health:ICONS.health,family:ICONS.family,mushrooms:ICONS.mushrooms,pet:ICONS.pet,
    sunglasses:ICONS.sunglasses,shopping:ICONS.shopping,electronics:ICONS.electronics,target:ICONS.target,
    wallet:ICONS.wallet,travel:ICONS.travel,beach:ICONS.beach,mountain:ICONS.mountain,
    bar:ICONS.bar,theatre:ICONS.theatre,
    ...Object.fromEntries([
      ['irlanda',ICONS.irlanda],['uk',ICONS.uk],['germania',ICONS.germania],['italia',ICONS.italia],
      ['spagna',ICONS.spagna],['francia',ICONS.francia],['portogallo',ICONS.portogallo],['austria',ICONS.austria],
      ['grecia',ICONS.grecia],['turchia',ICONS.turchia],['croazia',ICONS.croazia],
      ['repubblicaCeca',ICONS.repubblicaCeca],['ungheria',ICONS.ungheria],['polonia',ICONS.polonia],
      ['slovacchia',ICONS.slovacchia],['slovenia',ICONS.slovenia],['islanda',ICONS.islanda],
      ['norvegia',ICONS.norvegia],['svezia',ICONS.svezia],['danimarca',ICONS.danimarca],
      ['finlandia',ICONS.finlandia],['lituania',ICONS.lituania],['lettonia',ICONS.lettonia],
      ['estonia',ICONS.estonia],['belgio',ICONS.belgio],['romania',ICONS.romania],['bulgaria',ICONS.bulgaria],
      ['malta',ICONS.malta],['messico',ICONS.messico],['usa',ICONS.usa],['giappone',ICONS.giappone],
      ['cina',ICONS.cina],['canada',ICONS.canada],['thailandia',ICONS.thailandia],['vietnam',ICONS.vietnam],
      ['indonesia',ICONS.indonesia],['india',ICONS.india],['singapore',ICONS.singapore],['marocco',ICONS.marocco],
      ['egitto',ICONS.egitto],['sudafrica',ICONS.sudafrica],['brasile',ICONS.brasile],['cile',ICONS.cile],
      ['argentina',ICONS.argentina],['colombia',ICONS.colombia],['peru',ICONS.peru],['costaRica',ICONS.costaRica],
      ['cuba',ICONS.cuba],['giamaica',ICONS.giamaica],['bahamas',ICONS.bahamas],['filippine',ICONS.filippine],
      ['cambogia',ICONS.cambogia],['nuovaZelanda',ICONS.nuovaZelanda],['fiji',ICONS.fiji],
      ['portoRico',ICONS.portoRico],['hongKong',ICONS.hongKong],['malaysia',ICONS.malaysia],['australia',ICONS.australia]
    ])
  };
