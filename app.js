...
function weekBaseDate(){return new Date(`${state.dashboardMonth||currentMonth()}-03T12:00:00`)}
...
function dayCardMarkup(day,today=false){return `<div class="day-card ${today?'today':''}"><div class="day-name">${day.name}</div><div class="day-num">${day.day}</div><div class="day-month">${day.month}</div><div class="day-in positive">+€ ${weekAmount(day.inc)}</div><div class="day-out negative">-€ ${weekAmount(day.out)}</div><button class="day-plus" onclick="openTransaction('income','${day.date}')">＋</button></div>`}
function dashboard(){const ym=state.dashboardMonth||currentMonth(), inc=income(ym), out=expenses(ym), sav=inc-out;const cats={};monthTx(ym).filter(t=>t.type==='expense').forEach(t=>cats[t.category]=(cats[t.category]||0)+t.amount);const totalOut=out||1;const colors=['#7a21ff','#ff1675','#ff6247','#29e88b','#ffd34f','#ffe08a','#a93de7'];const entries=Object.entries(cats);const days=dashboardWeekDays();
return `<section class="page dashboard-page ${state.page==='dashboard'?'active':''}${dashboardWeekTransition?' week-changing':''}">
  <div class="hero">
    ...
    <select class="month-select" aria-label="Mese dashboard" onchange="setDashboardMonth(this.value)">${dashboardMonthOptions().map(m=>`<option value="${m}" ${m===ym?'selected':''}>${monthName(m)}</option>`).join('')}</select>
...
function forecasts(){const f=forecast(state.forecastMonth);return `<section class="page ${state.page==='forecasts'?'active':''}"><div class="page-head"><div><h1>Previsioni</h1></div><button class="icon-btn" onclick="openSettings()">⚙</button></div><div class="month-control"><button onclick="shiftForecast(-1)">‹</button><select onchange="state.forecastMonth=this.value;save();render()">${forecastMonthOptions().map(x=>`<option value="${x}" ${x===state.forecastMonth?'selected':''}>${monthName(x)}</option>`).join('')}</select><button onclick="shiftForecast(1)">›</button></div><div class="panel forecast-card"><h3 style="margin-top:0">Situazione prevista</h3><div class="forecast-row"><span>🟢 Entrate previste</span><b>${euroFmt.format(f.income)}</b></div><div class="forecast-row"><span>🔴 Uscite previste</span><b>${euroFmt.format(f.expenses)}</b></div><div class="forecast-row"><span>📉 Saldo mese</span><b class="${f.net>=0?'positive':'negative'}">${euroFmt.format(f.net)}</b></div><div class="forecast-row"><span>🏦 Totale stimato fine mese</span><b class="positive">${euroFmt.format(f.endBalance)}</b></div><div class="forecast-note">Queste previsioni tengono conto della media di tutti i mesi precedenti disponibili, applicando prudenzialmente <b>−5% alle entrate</b> e <b>+5% alle spese</b>, oltre agli eventi futuri inseriti.</div></div><div class="section-title">Eventi futuri <button class="primary-btn" onclick="openFutureEvent()">＋ Aggiungi evento</button></div><div class="events-list">${futureEventsForMonth(state.forecastMonth).map(eventRow).join('')||'<div class="empty">Nessun evento previsto per questo mese.</div>'}</div><div class="panel" style="margin-top:14px;padding:16px"><h3 style="margin:0">Andamento previsto dei risparmi</h3><div class="bar-chart">${forecastMonthOptions().slice(0,6).map(m=>{const x=forecast(m).endBalance;const h=Math.max(15,Math.min(100,x/5000*100));return `<div class="bar" style="height:${h}%"><span>${monthName(m).slice(0,3)}</span></div>`}).join('')}</div></div></section>`}
function monthKeyShift(ym,n){let [y,m]=ym.split('-').map(Number);m+=n;while(m>12){m-=12;y++}while(m<1){m+=12;y--}return `${y}-${String(m).padStart(2,'0')}`}
function historicalMonths(){const months=new Set();state.transactions.forEach(t=>months.add(monthOf(t.date)));state.futureEvents.forEach(e=>months.add(monthOf(e.date)));if(state.startingCapitalMonth)months.add(state.startingCapitalMonth);months.add(currentMonth());return [...months].sort()}
function forecastMonthOptions(){const months=new Set(historicalMonths());const seed=state.forecastMonth||currentMonth();for(let i=0;i<12;i++)months.add(monthKeyShift(seed,i));for(let i=1;i<=3;i++)months.add(monthKeyShift(seed,-i));return [...months].sort()}
function dashboardMonthOptions(){const months=new Set(forecastMonthOptions());months.add(state.dashboardMonth||currentMonth());return [...months].sort()}
function setDashboardMonth(ym){state.dashboardMonth=ym;dashboardWeekOffset=0;dashboardWeekTransition=null;save();render()}
function shiftForecast(n){const a=forecastMonthOptions(),i=a.indexOf(state.forecastMonth);const pos=i<0?0:i;state.forecastMonth=a[Math.max(0,Math.min(a.length-1,pos+n))];save();render()}
function futureEventsForMonth(ym){return state.futureEvents.filter(e=>monthOf(e.date)===ym)}
function averageActualMonth(type,beforeYm){const months=[...new Set(state.transactions.map(t=>monthOf(t.date)).filter(m=>m<beforeYm))].sort();if(!months.length){const fallback=currentMonth();return type==='income'?income(fallback):expenses(fallback)}const values=months.map(m=>type==='income'?income(m):expenses(m));return sum(values)/values.length}
function forecastBase(ym){return {income:averageActualMonth('income',ym)*.95,expenses:averageActualMonth('expense',ym)*1.05}}
function monthForecastNet(ym){const base=forecastBase(ym);const events=futureEventsForMonth(ym);const ei=sum(events.filter(e=>e.type==='income').map(e=>Number(e.amount)||0));const ee=sum(events.filter(e=>e.type==='expense').map(e=>Number(e.amount)||0));return {income:base.income+ei,expenses:base.expenses+ee,net:base.income+ei-base.expenses-ee}}
function forecast(ym){const current=currentMonth();const monthData=monthForecastNet(ym);let running=state.balance;if(ym>current){for(let m=monthKeyShift(current,1);m<=ym;m=monthKeyShift(m,1)){running+=monthForecastNet(m).net}}else if(ym<current){const actualMonths=[...new Set(state.transactions.map(t=>monthOf(t.date)).filter(m=>m>ym&&m<=current))].sort();for(const m of actualMonths){running-=savings(m)}}return {income:monthData.income,expenses:monthData.expenses,net:monthData.net,endBalance:running}}
...
function movements(){const items=[...state.transactions].sort((a,b)=>b.date.localeCompare(a.date)).filter(t=>state.movementFilter==='all'||t.type===state.movementFilter);return `<section class="page ${state.page==='movements'?'active':''}"><div class="page-head"><div><h1>Movimenti</h1></div><button class="primary-btn" onclick="openTransaction()">＋</button></div><div class="filters"><button class="filter-btn ${state.movementFilter==='all'?'active':''}" onclick="setFilter('all')">Tutti</button><button class="filter-btn ${state.movementFilter==='income'?'active':''}" onclick="setFilter('income')">Entrate</button><button class="filter-btn ${state.movementFilter==='expense'?'active':''}" onclick="setFilter('expense')">Uscite</button></div><div class="movement-list">${items.map(t=>`<div class="movement-row" data-transaction-id="${t.id}"><div class="event-icon">${t.type==='income'?'↗':'↘'}</div><div class="event-meta"><b>${escapeHtml(t.description)}</b><small>${t.category} · ${new Date(t.date+'T12:00').toLocaleDateString('it-IT')}</small></div><div class="event-actions"><strong class="movement-amount ${t.type==='income'?'positive':'negative'}">${t.type==='income'?'+':'-'}${euroFmt.format(t.amount)}</strong></div></div>`).join('')||'<div class="empty">Nessun movimento.</div>'}</div></section>`}
function render(){
  ...
  try{
    ...
    document.querySelectorAll('.movement-row[data-transaction-id]').forEach(row=>{let startX=0,startY=0,swiped=false;row.addEventListener('touchstart',e=>{if(e.touches.length!==1)return;startX=e.touches[0].clientX;startY=e.touches[0].clientY;swiped=false},{passive:true});row.addEventListener('touchend',e=>{const t=e.changedTouches[0];const dx=t.clientX-startX,dy=t.clientY-startY;if(dx<-70&&Math.abs(dx)>Math.abs(dy)*1.2){swiped=true;deleteTransaction(row.dataset.transactionId)}});row.addEventListener('click',()=>{if(!swiped)openTransactionForEdit(row.dataset.transactionId)})});
  }catch(err){
...
function openTransactionForEdit(id){const t=state.transactions.find(x=>x.id===id);if(!t)return;let type=t.type;openModal(`<div class="modal-head"><h2>Modifica movimento</h2><button class="close-btn" onclick="closeModal()">×</button></div><div class="type-switch"><button id="typeIncome" class="income ${type==='income'?'active':''}" onclick="selectType('income')">↓ Entrata</button><button id="typeExpense" class="expense ${type==='expense'?'active':''}" onclick="selectType('expense')">↑ Uscita</button></div><form id="transactionForm" class="form-grid"><div class="form-grid two"><div class="field"><label>Data</label><input name="date" type="date" value="${t.date}" required></div><div class="field"><label>Categoria</label><select id="transactionCategory" name="category">${categoriesForType(type).map(c=>`<option ${c===t.category?'selected':''}>${c}</option>`).join('')}</select></div></div><div class="field"><label>Importo</label><input name="amount" type="number" min="0.01" step="0.01" value="${t.amount}" required></div><div class="modal-actions"><button type="button" class="secondary-btn" onclick="closeModal()">Annulla</button><button class="primary-btn">Salva modifiche</button></div></form>`);const f=document.getElementById('transactionForm');f.dataset.type=type;f.dataset.editId=id;f.addEventListener('submit',submitTransaction)}
...
function submitTransaction(ev){ev.preventDefault();const f=ev.currentTarget,d=new FormData(f),amount=Number(d.get('amount'));const item={date:d.get('date'),type:f.dataset.type,category:d.get('category'),description:d.get('category'),amount};if(f.dataset.editId){const i=state.transactions.findIndex(x=>x.id===f.dataset.editId);if(i>=0)state.transactions[i]={...state.transactions[i],...item}}else state.transactions.push({id:uid(),...item});recalculateBalance();save();closeModal();render()}
...
function openFutureEvent(id=''){const existing=id?state.futureEvents.find(e=>e.id===id):null;let type=existing?.type||'expense';const date=existing?.date||`${state.forecastMonth}-15`;const category=existing?.category||futureCategoriesForType(type)[0];const description=existing?.description||'';const amount=existing?.amount??'';openModal(`<div class="modal-head"><h2>${existing?'Modifica evento previsto':'Nuovo evento previsto'}</h2><button class="close-btn" onclick="closeModal()">×</button></div><div class="type-switch"><button id="typeIncome" class="income ${type==='income'?'active':''}" onclick="selectFutureType('income')">↓ Entrata</button><button id="typeExpense" class="expense ${type==='expense'?'active':''}" onclick="selectFutureType('expense')">↑ Uscita</button></div><form id="futureForm" class="form-grid"><div class="form-grid two"><div class="field"><label>Data</label><input name="date" type="date" value="${date}" required></div><div class="field"><label>Categoria</label><select id="futureCategory" name="category">${futureCategoriesForType(type).map(c=>`<option ${c===category?'selected':''}>${c}</option>`).join('')}</select></div></div><div class="field"><label>Descrizione</label><input name="description" value="${escapeHtml(description)}" required></div><div class="field"><label>Importo</label><input name="amount" type="number" min="0.01" step="0.01" value="${amount}" required></div><div class="modal-actions"><button type="button" class="secondary-btn" onclick="closeModal()">Annulla</button><button class="primary-btn">${existing?'Salva modifiche':'Salva evento'}</button></div></form>`);const f=document.getElementById('futureForm');f.dataset.type=type;if(existing)f.dataset.editId=existing.id;f.addEventListener('submit',submitFuture)}
...
function submitFuture(ev){ev.preventDefault();const f=ev.currentTarget,d=new FormData(f);const item={date:d.get('date'),type:f.dataset.type,category:d.get('category'),description:d.get('description'),amount:Number(d.get('amount'))};if(f.dataset.editId){const i=state.futureEvents.findIndex(e=>e.id===f.dataset.editId);if(i>=0)state.futureEvents[i]={...state.futureEvents[i],...item}}else state.futureEvents.push({id:uid(),...item});save();closeModal();render()}
