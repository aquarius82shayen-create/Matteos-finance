...
function changePage(page){
  ...
  if(page==='forecasts') state.forecastMonth=currentMonth();
  save();
  render();
...
function futureEventsConsultation(){
...
function futureEventsConsultationMarkup(){
  const count=futureEventsConsultation().length;
  return `<button type="button" class="panel future-events-consult-panel" onclick="changePage('future-events')" aria-label="Visualizza tutti gli eventi futuri"><span class="future-events-consult-icon">📅</span><span class="future-events-consult-copy"><b>TUTTI GLI EVENTI FUTURI</b><small>${count ? `${count} ${count===1?'evento':'eventi'} programmati` : 'Nessun evento futuro programmato'}</small></span><span class="future-events-consult-arrow">›</span></button>`;
}
function futureEventsPage(){
  const events=futureEventsConsultation();
  const rows=events.map(e=>{
    const tone=e.type==='income'?'income':'expense';
    const sign=e.type==='income'?'+':'-';
    const date=new Date(e.date+'T12:00:00').toLocaleDateString('it-IT',{day:'numeric',month:'long',year:'numeric'});
    return `<div class="future-events-page-row event-${tone}"><div class="event-icon">${eventIcon(e.description,e.category,e.type)}</div><div class="event-meta"><b>${escapeHtml(e.description||'Evento')}</b><small>${escapeHtml(date)} · ${escapeHtml(e.category||'')}</small></div><strong class="${tone==='income'?'positive':'negative'}">${sign}${euroFmt.format(Number(e.amount)||0)}</strong></div>`;
  }).join('');
  return `<section class="page ${state.page==='future-events'?'active':''}"><div class="page-head future-events-page-head"><button type="button" class="icon-btn" onclick="changePage('forecasts')" aria-label="Torna a Previsioni">‹</button><div><h1>Tutti gli eventi futuri</h1><small>${events.length} ${events.length===1?'evento':'eventi'} programmati</small></div><span class="future-events-head-spacer"></span></div><div class="future-events-page-list">${rows||'<div class="empty">Nessun evento futuro programmato.</div>'}</div></section>`;
}
...
const BUILTIN_ICON_LIBRARY_FILES=[
  "1_viaggi_viaggio.webp",
  "2_aereo_biglietto aereo_ .webp",
  "3_mare_spiaggia_sardegna_relax.webp",
  "4_montagna_viaggio in montagna_trekking.webp",
  "5_hotel_alloggio_albergo.webp",
  "6_campeggio_tenda_.webp",
  "7_concerto_metal.webp",
  "8_cinema_film_movie_netflix_prime.webp",
  "9_teatro_opera_lirico.webp",
  "10_palestra_fix fit_.webp",
  "11_automobile_macchina_auto.webp",
  "12_meccanico.webp",
  "13_guasto_auto.webp",
  "14_benzina_pieno_diesel.webp",
  "16_treno.webp",
  "17_nave_battello_ferry_crocera.webp",
  "18_casa.webp",
  "19_elettrodomestici_lavatrice_frigorifero.webp",
  "20_gas_bombola.webp",
  "21_luce_lampadina_neon_illuminazione_faro_torcia.webp",
  "22_internet_connessione_modem_wind_iliad_vodafone.webp",
  "23_bollette_bolletta.webp",
  "24_donazione.webp",
  "25_trasloco.webp",
  "26_riparazione.webp",
  "27_regalo_regalino_pensiero.webp",
  "28_amore_san valentino.webp",
  "29_salute.webp",
  "30_medico_visita medica_fisioterapia.webp",
  "31_dentista_otturazione_dentale.webp",
  "32_mamma.webp",
  "33_elettrocardiogramma_radiografia_ecografia.webp",
  "34_funghi_allucinogeni.webp",
  "35_gatto.webp",
  "36_veterinario.webp",
  "37_farmacia_farmaci_sciroppo_medicine_medicina.webp",
  "38_occhiali_da_sole.webp",
  "39_acquisti_importanti.webp",
  "40_tecnologia_cellulare_pc_tablet.webp",
  "41_casa.webp",
  "42_obiettivo.webp",
  "43_rimborso.webp",
  "44_party_festa.webp",
  ...
  "52_compleanno.webp",
  "53_matrimonio.webp",
  "54_lutto_funere_morte.webp",
  "55_tasse_tassa_imposte.webp",
  "56_multa_multe.webp",
  "57_carta_di_credito.webp",
  "58_bollo_auto.webp",
  "59_assicurazione_auto.webp",
  "60_sud_america.webp",
  "61_bar.webp",
  "62_ristorante_cena_pranzo.webp",
  "63_nord_america.webp",
  "64_africa.webp",
  "65_asia.webp",
  "66_oceania.webp",
  "67_inghilterra_londra.webp",
  "68_thailandia.webp",
  "69_cina.webp",
  "70_malesia.webp",
  "71_canada.webp",
  ...
  "93_filippine.webp",
  "94_cambogia.webp",
  "95_nuova_zelanda.webp",
  "96_fiji.webp",
  "97_piante_pianta_terra.webp",
  "100_bilancia_bilanciare_equilibrare_pareggiare_giustizia_pareggio.webp",
  "101_Messico.webp",
  "102_Stati_Uniti.webp",
  "103_Lituania.webp",
  "104_Lettonia.webp",
  "105_Estonia.webp",
  "106_Slovacchia.webp",
  ...
  "138_stadio.webp",
  "139_sport.webp",
  "140_museo_musei.webp",
  "141_pallavolo_volley..webp",
  "142_calcio.webp",
  "143_basket.webp",
  "144_surf.webp",
  "146_rugby.webp",
  "147_libri_libro.webp",
  "148_musica_suonare_prove.webp",
  "149_aperitivo.webp",
  "150_pizza.webp",
  "151_cucina_cinese.webp",
  "152_hamburger.webp",
  "153_irlanda.webp",
  "190_puttana_puttane_mignotte_pompino_puttan tour.webp",
  "191_mc donald.webp",
  "192_deliveroo_asporto.webp",
  "193_bombola_bombole.webp",
  "194_iron_maiden.webp",
  "195_indumenti_maglia_maglione_jeans_pantaloni_intimo_camicia.webp"
];;
let ICON_LIBRARY_FILES=[...BUILTIN_ICON_LIBRARY_FILES];
const ICON_LIBRARY_CACHE_KEY='matteos-finance-icon-library-v2';
const ICON_LIBRARY_CACHE_TTL=30*1000;
let iconLibraryLoading=false;
function iconLibraryApiUrl(){
  const fallback='https://api.github.com/repos/aquarius82shayen-create/Matteos-finance/contents/assets/icons?ref=main';
  try{
    const host=window.location.hostname||'';
    const parts=window.location.pathname.split('/').filter(Boolean);
    if(host.endsWith('.github.io')){
      const owner=host.split('.')[0];
      const repo=parts[0]||'Matteos-finance';
      return `https://api.github.com/repos/${owner}/${repo}/contents/assets/icons?ref=main`;
    }
  }catch(e){}
  return fallback;
}
function readCachedIconLibrary(){
  try{
    const cached=JSON.parse(localStorage.getItem(ICON_LIBRARY_CACHE_KEY)||'null');
    if(Array.isArray(cached?.files)&&cached.files.length){
      ICON_LIBRARY_FILES=cached.files.filter(f=>/\.webp$/i.test(f));
      return true;
    }
  }catch(e){}
  return false;
...
async function loadIconLibrary(){
  ...
  const hadCache=readCachedIconLibrary();
  try{
    const cached=JSON.parse(localStorage.getItem(ICON_LIBRARY_CACHE_KEY)||'null');
    const fresh=cached?.savedAt&&Date.now()-cached.savedAt<ICON_LIBRARY_CACHE_TTL;
    if(fresh){
      render();
      return;
    }
  ...
  try{
    const response=await fetch(iconLibraryApiUrl(),{headers:{'Accept':'application/vnd.github+json'}});
    if(!response.ok)throw new Error(`GitHub icons HTTP ${response.status}`);
    const items=await response.json();
    const files=Array.isArray(items)?items.filter(x=>x&&x.type==='file'&&/\.webp$/i.test(x.name)).map(x=>x.name):[];
    if(!files.length)throw new Error('Libreria icone vuota');
    files.sort((a,b)=>a.localeCompare(b,undefined,{numeric:true,sensitivity:'base'}));
    ICON_LIBRARY_FILES=files;
    localStorage.setItem(ICON_LIBRARY_CACHE_KEY,JSON.stringify({savedAt:Date.now(),files}));
  }catch(error){
    console.warn('Libreria icone remota non disponibile; uso la copia locale disponibile.',error);
    if(!hadCache)ICON_LIBRARY_FILES=[...BUILTIN_ICON_LIBRARY_FILES];
  }finally{
...
const ICONS={
  travel:"1_viaggi_viaggio.webp",
  flight:"2_aereo_biglietto aereo_ .webp",
  beach:"3_mare_spiaggia_sardegna_relax.webp",
  mountain:"4_montagna_viaggio in montagna_trekking.webp",
  hotel:"5_hotel_alloggio_albergo.webp",
  camping:"6_campeggio_tenda_.webp",
  concert:"7_concerto_metal.webp",
  bar:"61_bar.webp",
  theatre:"9_teatro_opera_lirico.webp",
  gym:"10_palestra_fix fit_.webp",
  car:"11_automobile_macchina_auto.webp",
  repair:"12_meccanico.webp",
  carIssue:"13_guasto_auto.webp",
  fuel:"14_benzina_pieno_diesel.webp",
  driving:"11_automobile_macchina_auto.webp",
  train:"16_treno.webp",
  cruise:"17_nave_battello_ferry_crocera.webp",
  house:"18_casa.webp",
  appliances:"19_elettrodomestici_lavatrice_frigorifero.webp",
  gas:"20_gas_bombola.webp",
  light:"21_luce_lampadina_neon_illuminazione_faro_torcia.webp",
  internet:"22_internet_connessione_modem_wind_iliad_vodafone.webp",
  bills:"23_bollette_bolletta.webp",
  charity:"24_donazione.webp",
  moving:"25_trasloco.webp",
  tools:"26_riparazione.webp",
  gift:"27_regalo_regalino_pensiero.webp",
  love:"28_amore_san valentino.webp",
  health:"29_salute.webp",
  doctor:"30_medico_visita medica_fisioterapia.webp",
  dentist:"31_dentista_otturazione_dentale.webp",
  family:"32_mamma.webp",
  healthHeart:"33_elettrocardiogramma_radiografia_ecografia.webp",
  mushrooms:"34_funghi_allucinogeni.webp",
  pet:"35_gatto.webp",
  vet:"36_veterinario.webp",
  pharmacy:"37_farmacia_farmaci_sciroppo_medicine_medicina.webp",
  sunglasses:"38_occhiali_da_sole.webp",
  shopping:"39_acquisti_importanti.webp",
  electronics:"40_tecnologia_cellulare_pc_tablet.webp",
  house2:"41_casa.webp",
  target:"42_obiettivo.webp",
  wallet:"43_rimborso.webp",
  party:"44_party_festa.webp",
  holiday:"45_ferie.webp",
  christmas:"46_natale.webp",
  newYear:"47_capodanno.webp",
  easter:"48_pasqua.webp",
  ferragosto:"49_ferragosto.webp",
  restaurant:"62_ristorante_cena_pranzo.webp",
  dinner:"62_ristorante_cena_pranzo.webp",
  aperitivo:"149_aperitivo.webp",
  wedding:"53_matrimonio.webp",
  mourning:"54_lutto_funere_morte.webp",
  taxes:"55_tasse_tassa_imposte.webp",
  multa:"56_multa_multe.webp",
  creditCard:"57_carta_di_credito.webp",
  carTax:"58_bollo_auto.webp",
  carInsurance:"59_assicurazione_auto.webp",
  messico:"101_Messico.webp",
  usa:"102_Stati_Uniti.webp",
  lituania:"103_Lituania.webp",
  lettonia:"104_Lettonia.webp",
  estonia:"105_Estonia.webp",
  slovacchia:"106_Slovacchia.webp",
  albania:"107_Albania.webp",
  islanda:"108_Islanda.webp",
  finlandia:"109_Finlandia.webp",
  giappone:"110_Giappone.webp",
  norvegia:"111_Norvegia.webp",
  svezia:"112_Svezia.webp",
  danimarca:"113_Danimarca.webp",
  polonia:"114_Polonia.webp",
  belgio:"121_Belgio.webp",
  svizzera:"122_Svizzera.webp",
  romania:"123_Romania.webp",
  bulgaria:"124_Bulgaria.webp",
  slovenia:"125_Slovenia.webp",
  malta:"126_Malta.webp",
  ungheria:"127_Ungheria.webp",
  repubblicaCeca:"128_Repubblica_Ceca.webp",
  croazia:"129_Croazia.webp",
  portogallo:"130_Portogallo.webp",
  austria:"131_austria.webp",
  grecia:"132_grecia.webp",
  turchia:"133_turchia.webp",
  italia:"134_italia.webp",
  spagna:"135_spagna.webp",
  francia:"136_francia.webp",
  germania:"137_germania.webp",
  stadio:"138_stadio.webp",
  sport:"139_sport.webp",
  museo:"140_museo_musei.webp",
  pallavolo:"141_pallavolo_volley..webp",
  calcio:"142_calcio.webp",
  basket:"143_basket.webp",
  surf:"144_surf.webp",
  bilancia:"100_bilancia_bilanciare_equilibrare_pareggiare_giustizia_pareggio.webp",
  rugby:"146_rugby.webp",
  libri:"147_libri_libro.webp",
  musica:"148_musica_suonare_prove.webp",
  pizza:"150_pizza.webp",
  cucinaCinese:"151_cucina_cinese.webp",
  hamburger:"152_hamburger.webp",
  irlanda:"153_irlanda.webp",
  thailandia:"68_thailandia.webp",
  cina:"69_cina.webp",
  malaysia:"70_malesia.webp",
  canada:"71_canada.webp",
  hongKong:"72_hong_kong.webp",
  australia:"73_australia.webp",
  vietnam:"74_vietnam.webp",
  india:"76_india.webp",
  singapore:"77_singapore.webp",
  marocco:"78_marocco.webp",
  egitto:"79_egitto.webp",
  sudafrica:"80_sudafrica.webp",
  repubblicaDominicana:"81_repubblica_dominicana.webp",
  brasile:"82_brasile.webp",
  portoRico:"83_porto_rico.webp",
  cile:"84_cile.webp",
  argentina:"85_argentina.webp",
  colombia:"86_colombia.webp",
  peru:"87_peru.webp",
  costaRica:"88_costa_rica.webp",
  cuba:"89_cuba.webp",
  russia:"90_russia.webp",
  giamaica:"91_giamaica.webp",
  bahamas:"92_bahamas.webp",
  filippine:"93_filippine.webp",
  cambogia:"94_cambogia.webp",
  nuovaZelanda:"95_nuova_zelanda.webp",
  fiji:"96_fiji.webp",
  piante:"97_piante_pianta_terra.webp",
};
...
function normalizeIconText(value=''){return String(value||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');}

/* Motore icone: il nome del file .webp è il vocabolario della libreria.
   1) nomi/entità specifiche; 2) luoghi; 3) concetti ricavati automaticamente dal nome del file;
   4) freccia. Aggiungere una parola al nome di un'icona amplia quindi automaticamente i suoi match,
   senza dover modificare questo codice. */
function semanticIconFile(description='',category='',type=''){
  const raw=String(description||'').trim(); if(!raw)return '';
  const normalize=(v='')=>String(v||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[’']/g,"'").replace(/[^\p{L}\p{N}\s]/gu,' ').replace(/\s+/g,' ').trim();
  const text=normalize(raw), compact=v=>normalize(v).replace(/\s+/g,'');
  const position=(v)=>{
    const p=normalize(v); if(!p)return Infinity;
    const escaped=p.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
    const re=new RegExp(`(?:^|\\s)${escaped}(?=$|\\s)`,'i');
    const m=text.match(re); if(m)return m.index+(m[0].length-p.length);
    ...
    for(let i=0;i<=textTokens.length-phraseTokens.length;i++){
      let ok=true;
      for(let j=0;j<phraseTokens.length;j++)if(textTokens[i+j]!==phraseTokens[j]){ok=false;break;}
      if(ok)return text.indexOf(textTokens.slice(0,i).join(' '));
    }
    return Infinity;
  };
  const idOf=f=>Number((f.match(/^\d+/)||['0'])[0]);
  const stop=new Set(['e','a','ad','al','alla','alle','ai','agli','da','dal','dalla','di','del','della','dei','degli','in','per','con','su','il','lo','la','i','gli','le','un','una','uno','the','and','of','to','per','nel','nella','nei','nelle','un','una']);
  const label=f=>normalize(f.replace(/^\d+_/,'').replace(/\.webp$/i,'').replace(/[_-]+/g,' '));
  const tokens=f=>label(f).split(' ').filter(w=>w.length>2&&!stop.has(w));
  const byId=new Map(ICON_LIBRARY_FILES.map(f=>[idOf(f),f]));

  // 1. Nomi/marchi/artisti specifici. La posizione nella descrizione risolve i pari livello.
  // Gli alias sono ricavati anche dal nome file, quindi aggiungere parole al filename è sufficiente.
  const namedIds=[190,191,192,194]; const named=[];
  for(const id of namedIds){
    const f=byId.get(id); if(!f)continue;
    const aliases=[label(f)];
    if(id===191)aliases.push('mcdonald',"mcdonald's",'mc donald');
    if(id===192)aliases.push('deliveroo');
    if(id===194)aliases.push('iron maiden');
    if(id===190)aliases.push('puttana','puttane','mignotte','pompino');
    const hits=aliases.map(x=>({x,pos:position(x)})).filter(x=>x.pos<Infinity);
    if(hits.length){const h=hits.sort((a,b)=>a.pos-b.pos||b.x.length-a.x.length)[0];named.push({file:f,pos:h.pos,len:normalize(h.x).length});}
  }
  if(named.length){named.sort((a,b)=>a.pos-b.pos||b.len-a.len);return named[0].file;}

  // 2. Paesi/località: prevalgono sui concetti generici.
  const places=[['irlanda',['dublino','irlanda'],ICONS.irlanda],['uk',['londra','inghilterra','regno unito','uk','united kingdom'],byId.get(67)],['germania',['berlino','germania'],ICONS.germania],['italia',['roma','italia','sardegna','valledoria'],ICONS.italia],['spagna',['madrid','spagna'],ICONS.spagna],['francia',['parigi','francia'],ICONS.francia],['portogallo',['lisbona','portogallo'],ICONS.portogallo],['austria',['vienna','austria'],ICONS.austria],['grecia',['atene','grecia'],ICONS.grecia],['turchia',['istanbul','turchia'],ICONS.turchia],['croazia',['zagabria','croazia'],ICONS.croazia],['repubblicaCeca',['praga','repubblica ceca'],ICONS.repubblicaCeca],['ungheria',['budapest','ungheria'],ICONS.ungheria],['polonia',['varsavia','polonia'],ICONS.polonia],['slovacchia',['bratislava','slovacchia'],ICONS.slovacchia],['slovenia',['lubiana','slovenia'],ICONS.slovenia],['islanda',['reykjavik','islanda'],ICONS.islanda],['norvegia',['oslo','norvegia'],ICONS.norvegia],['svezia',['stoccolma','svezia'],ICONS.svezia],['danimarca',['copenaghen','danimarca'],ICONS.danimarca],['finlandia',['helsinki','finlandia'],ICONS.finlandia],['lituania',['vilnius','lituania'],ICONS.lituania],['lettonia',['riga','lettonia'],ICONS.lettonia],['estonia',['tallinn','estonia'],ICONS.estonia],['belgio',['bruxelles','belgio'],ICONS.belgio],['romania',['bucarest','bucharest','romania'],ICONS.romania],['bulgaria',['sofia','bulgaria'],ICONS.bulgaria],['malta',['malta'],ICONS.malta],['messico',['messico'],ICONS.messico],['usa',['stati uniti','usa','new york','america'],ICONS.usa],['giappone',['giappone','tokyo','kyoto'],ICONS.giappone],['cina',['cina','pechino','shanghai'],ICONS.cina],['canada',['canada','toronto','vancouver'],ICONS.canada],['thailandia',['thailandia','bangkok'],ICONS.thailandia],['vietnam',['vietnam'],ICONS.vietnam],['india',['india'],ICONS.india],['singapore',['singapore'],ICONS.singapore],['marocco',['marocco'],ICONS.marocco],['egitto',['egitto'],ICONS.egitto],['sudafrica',['sudafrica','sud africa'],ICONS.sudafrica],['brasile',['brasile'],ICONS.brasile],['cile',['cile'],ICONS.cile],['argentina',['argentina'],ICONS.argentina],['colombia',['colombia'],ICONS.colombia],['peru',['peru'],ICONS.peru],['costaRica',['costa rica'],ICONS.costaRica],['cuba',['cuba'],ICONS.cuba],['giamaica',['giamaica'],ICONS.giamaica],['bahamas',['bahamas'],ICONS.bahamas],['filippine',['filippine'],ICONS.filippine],['cambogia',['cambogia'],ICONS.cambogia],['nuovaZelanda',['nuova zelanda'],ICONS.nuovaZelanda],['portoRico',['porto rico'],ICONS.portoRico],['hongKong',['hong kong'],ICONS.hongKong],['malaysia',['malaysia'],ICONS.malaysia],['australia',['australia'],ICONS.australia]];
  const ph=[];for(const [,aliases,f] of places){if(!f)continue;for(const x of aliases){const p=position(x);if(p<Infinity){ph.push({file:f,pos:p,len:normalize(x).length});break;}}}if(ph.length){ph.sort((a,b)=>a.pos-b.pos||b.len-a.len);return ph[0].file;}

  // 3. Vocabolario automatico dai filename.
  // Più parole condivise = icona più specifica; una frase di più parole vale più di un singolo termine.
  // La posizione nella descrizione risolve i pari livello. I termini molto generici non vengono premiati.
  const generic=new Set(['casa','viaggio','viaggi','cibo','cena','pranzo','musica','sport','auto','macchina','persona','persone','cose','acquisto','acquisti','evento','eventi']);
  const candidates=[];
  for(const f of ICON_LIBRARY_FILES){
    const ts=tokens(f); if(!ts.length)continue;
    const hits=ts.filter(t=>position(t)<Infinity); if(!hits.length)continue;
    let phrase=0, matchedPhrase='';
    // Controlla sequenze di 2-5 parole del filename presenti nella descrizione.
    for(let i=0;i<ts.length;i++)for(let j=Math.min(ts.length,i+5);j>=i+2;j--){
      const p=ts.slice(i,j).join(' '); if(p.length>3&&text.includes(p)){if(p.length>phrase){phrase=p.length;matchedPhrase=p;}break;}
    }
    const specificHits=hits.filter(t=>!generic.has(t));
    const score=specificHits.length*100 + hits.length*10 + phrase*2;
    candidates.push({file:f,score,count:hits.length,phrase,pos:Math.min(...hits.map(position)),id:idOf(f),matchedPhrase});
  }
  if(candidates.length){
    candidates.sort((a,b)=>b.score-a.score||b.phrase-a.phrase||a.pos-b.pos||a.id-b.id);
...
function goals(){const active=activeGoals(),completed=completedGoals(),archived=archivedGoals(),showCompleted=state.goalTab==='completed',showArchived=state.goalTab==='archived';const archivedRows=archived.map(g=>`<div class="archived-goal-row" role="button" tabindex="0" onclick="openArchivedGoal('${g.id}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openArchivedGoal('${g.id}')}"><span class="archived-goal-name">${escapeHtml(g.name)}</span><button type="button" class="archived-goal-delete" aria-label="Cancella definitivamente ${escapeHtml(g.name)}" onclick="event.stopPropagation();requestDeleteArchivedGoal('${g.id}')">×</button></div>`).join('');return `<section class="page ${state.page==='goals'?'active':''}"><div class="page-head"><div><h1>Obiettivi</h1></div><button class="icon-btn" onclick="openSettings()">⚙</button></div><button class="primary-btn" style="width:100%;font-size:18px" onclick="openGoal()">＋ Nuovo obiettivo</button>${dataQualityWarningMarkup()}<div class="goal-tabs"><button class="${!showCompleted&&!showArchived?'active':''}" onclick="setGoalTab('active')">In corso (${active.length})</button><button id="completedGoalsTab" class="${showCompleted?'active':''}" onclick="setGoalTab('completed')">Completati (${completed.length})</button><button class="${showArchived?'active':''}" onclick="setGoalTab('archived')">Archiviati (${archived.length})</button></div><div class="goals-list">${showArchived?(archivedRows||'<div class="empty">Nessun obiettivo archiviato.</div>'):showCompleted?(completed.map(completedGoalCard).join('')||'<div class="empty">Nessun obiettivo completato.</div>'):(active.map(goalCard).join('')||'<div class="empty">Non hai ancora obiettivi in corso.</div>')}</div></section>`}
...
function requestDeleteArchivedGoal(id){const g=state.goals.find(x=>x.id===id);if(!g||!g.completed||!g.archived)return;openModal(`<div class="confirm-card"><div class="confirm-icon">🗑</div><h2>Cancellazione definitiva</h2><p>Stai cancellando definitivamente questo obiettivo dall'archivio.</p><p><b>${escapeHtml(g.name)}</b></p><div class="modal-actions confirm-actions"><button type="button" class="secondary-btn" onclick="closeModal()">Annulla</button><button type="button" class="danger-btn" onclick="deleteArchivedGoal('${id}')">Continua</button></div></div>`)}
function deleteArchivedGoal(id){const g=state.goals.find(x=>x.id===id);if(!g||!g.completed||!g.archived)return;state.goals=state.goals.filter(x=>x.id!==id);state.goalTab='archived';save();closeModal();render()}
...
function render(){
  ...
  try{
    ...
    const pages={dashboard,movements,forecasts,goals,'future-events':futureEventsPage};
