...
function forecastChartMarkup(){
  ...
  const W=600,H=230,padX=34,padY=28;
  ...
  const guideValues=[min,(min+max)/2,max].filter((v,i,a)=>i===0||Math.abs(v-a[i-1])>.01);
  const guides=guideValues.map(v=>`<g><line x1="${padX}" x2="${W-padX}" y1="${y(v).toFixed(1)}" y2="${y(v).toFixed(1)}" class="forecast-guide"/><text x="2" y="${(y(v)+4).toFixed(1)}" class="forecast-guide-label">${euroFmt.format(v)}</text></g>`).join('');
  const labels=data.map((d,i)=>`<div class="forecast-chart-label" style="left:${(x(i)/W*100).toFixed(2)}%"><span>${monthName(d.m).slice(0,3)}</span><b>${euroFmt.format(d.value)}</b></div>`).join('');
  return `<div class="forecast-chart-wrap"><div class="forecast-chart-summary"><span>Minimo <b>${euroFmt.format(Math.min(...values))}</b></span><span>Finale <b>${euroFmt.format(values.at(-1))}</b></span><span>🛡 Cuscinetto <b>${euroFmt.format(cushion)}</b></span></div><div class="forecast-chart"><svg viewBox="0 0 ${W} ${H}" preserveAspectRatio="none" aria-label="Andamento del saldo previsto">${guides}<line x1="${padX}" x2="${W-padX}" y1="${cushionY}" y2="${cushionY}" class="cushion-line"/><polyline points="${points}" class="forecast-line"/>${data.map((d,i)=>`<circle cx="${x(i).toFixed(1)}" cy="${y(d.value).toFixed(1)}" r="5" class="forecast-point"><title>${monthName(d.m)}: ${euroFmt.format(d.value)}</title></circle>`).join('')}</svg><div class="forecast-chart-labels">${labels}</div></div></div>`;
}
...
function movements(){
  ...
  const items=state.transactions.map((t,index)=>({t,index})).filter(({t})=>(state.movementMonth==='all'||monthOf(t.date)===state.movementMonth)&&(state.movementFilter==='all'||t.type===state.movementFilter)).sort((a,b)=>b.t.date.localeCompare(a.t.date)||b.index-a.index).map(({t})=>t);
...
function openModal(content){document.getElementById('modalRoot').innerHTML=`<div class="modal-backdrop" onclick="if(event.target===this)closeModal()"><div class="modal">${content}</div></div>`;enhanceFinanceControls(document.getElementById('modalRoot'))}
...
function selectType(type){const f=document.getElementById('transactionForm');f.dataset.type=type;document.getElementById('typeIncome').classList.toggle('active',type==='income');document.getElementById('typeExpense').classList.toggle('active',type==='expense');const category=document.getElementById('transactionCategory');if(category){category.innerHTML=categoryOptions(type);refreshFinanceControl(category);syncTransactionDescriptionRequirement();}}
function submitTransaction(ev){
  ...
  if(canonicalCategory(data.category)==='Obiettivi'&&!description){financeAlert('Per un movimento Obiettivi la descrizione è obbligatoria.',{title:'Descrizione obbligatoria'});return;}
  const existingId=f.dataset.editId;
  const previous=existingId?state.transactions.find(t=>t.id===existingId):null;
  const transaction=existingId?{...previous,...data}:{id:uid(),...data};
  ...
  if(match){
    ...
    financeConfirm(`Hai appena registrato: ${transaction.description||transaction.category} — ${euroFmt.format(transaction.amount)}

Potrebbe corrispondere all'evento futuro: ${event.description} — ${euroFmt.format(event.amount)}

Era per caso l'evento futuro che stavamo aspettando?`,()=>{state.futureEvents=state.futureEvents.filter(e=>e.id!==event.id);finalize();},{title:'Possibile evento futuro trovato',confirmText:'Conferma'} ,()=>finalize());
    return;
  }
  finalize();
}
...
function selectFutureType(type){const f=document.getElementById('futureForm');f.dataset.type=type;document.getElementById('typeIncome').classList.toggle('active',type==='income');document.getElementById('typeExpense').classList.toggle('active',type==='expense');const category=document.getElementById('futureCategory');if(category){category.innerHTML=futureCategoryOptions(type);refreshFinanceControl(category)}}
...
function importBackup(event){
  ...
  reader.onload=()=>{
    try{
      ...
      financeConfirm('Importando il backup i dati attuali verranno sostituiti. Vuoi continuare?',()=>{
        const safe={...structuredClone(DEFAULT_STATE),...imported,transactions:Array.isArray(imported.transactions)?imported.transactions:[],futureEvents:Array.isArray(imported.futureEvents)?imported.futureEvents:[],goals:Array.isArray(imported.goals)?imported.goals:[],cushionMinimum:Math.max(0,Number(imported.cushionMinimum)||0),cushionIdeal:Math.max(0,Number(imported.cushionIdeal)||0)};
        Object.keys(state).forEach(key=>delete state[key]);
        Object.assign(state,safe);
        recalculateBalance();save();closeModal();render();
        setTimeout(()=>financeAlert('Backup importato correttamente.',{title:'Importazione completata'}),0);
      },{title:'Importare il backup?',confirmText:'Importa',danger:true});
    }catch(err){
      financeAlert('Impossibile importare il backup: il file non è valido.',{title:'Errore di importazione',danger:true});
...
function confirmDeleteMonth(){const m=document.getElementById('deleteMonth').value;if(!m)return;financeConfirm('Vuoi davvero cancellare tutti i dati di '+monthName(m)+'?',()=>{const removed=state.transactions.filter(t=>monthOf(t.date)===m);state.transactions=state.transactions.filter(t=>monthOf(t.date)!==m);restoreGoalsLinkedToTransactions(removed);recalculateBalance();save();closeModal();render()},{title:'Cancellare il mese?',danger:true})}
function confirmDeleteAllTransactions(){financeConfirm('Vuoi davvero cancellare tutte le entrate e le uscite?',()=>{const removed=[...state.transactions];state.transactions=[];restoreGoalsLinkedToTransactions(removed);recalculateBalance();save();closeModal();render()},{title:'Cancellare tutti i movimenti?',danger:true})}
function confirmResetMonth(){const m=currentMonth();financeConfirm('Vuoi davvero cancellare i dati del mese corrente?',()=>{const removed=state.transactions.filter(t=>monthOf(t.date)===m);state.transactions=state.transactions.filter(t=>monthOf(t.date)!==m);restoreGoalsLinkedToTransactions(removed);state.futureEvents=state.futureEvents.filter(e=>monthOf(e.date)!==m);recalculateBalance();save();closeModal();render()},{title:'Reset mese?',danger:true})}
function confirmResetTotal(){financeConfirm('Vuoi davvero cancellare TUTTI i dati? Questa operazione non si può annullare.',()=>{state.balance=0;state.startingCapital=0;state.startingCapitalMonth='';state.transactions=[];state.futureEvents=[];state.goals=[];state.cushionMinimum=0;state.cushionIdeal=0;save();closeModal();render()},{title:'Reset totale?',danger:true})}
...
function resetDemo(){financeConfirm('Ripristinare i dati iniziali?',()=>{localStorage.removeItem('matteos-finance-v1');location.reload()},{title:'Ripristino dati',danger:true})}
function escapeHtml(s){return String(s).replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]))}
render();


/* Controlli e finestre in stile MatteosFinance: sostituiscono le finestre native senza modificare la struttura delle schermate. */
function financeAlert(message,options={}){const title=options.title||'Matteo\'s Finance',danger=options.danger?' danger':'';const root=document.createElement('div');root.className='finance-window-backdrop';root.innerHTML=`<div class="finance-window finance-alert${danger}"><div class="finance-window-icon">${options.danger?'!':'✓'}</div><h2>${escapeHtml(title)}</h2><p>${escapeHtml(message).replace(/\n/g,'<br>')}</p><div class="finance-window-actions"><button class="primary-btn" type="button">Ok</button></div></div>`;const close=()=>root.remove();root.querySelector('button').onclick=close;root.addEventListener('click',e=>{if(e.target===root)close()});document.body.appendChild(root)}
function financeConfirm(message,onConfirm,options={},onCancel){const title=options.title||'Conferma',danger=options.danger?' danger':'';const confirmText=options.confirmText||'Conferma';const root=document.createElement('div');root.className='finance-window-backdrop';root.innerHTML=`<div class="finance-window finance-confirm${danger}"><div class="finance-window-icon">?</div><h2>${escapeHtml(title)}</h2><p>${escapeHtml(message).replace(/\n/g,'<br>')}</p><div class="finance-window-actions"><button class="secondary-btn" type="button">Annulla</button><button class="${options.danger?'danger-btn':'primary-btn'}" type="button">${escapeHtml(confirmText)}</button></div></div>`;const buttons=root.querySelectorAll('button');const cancel=()=>{root.remove();if(typeof onCancel==='function')onCancel()};buttons[0].onclick=cancel;buttons[1].onclick=()=>{root.remove();if(typeof onConfirm==='function')onConfirm()};root.addEventListener('click',e=>{if(e.target===root)cancel()});document.body.appendChild(root)}
function financeDateLabel(value){if(!value)return 'Seleziona data';const d=new Date(value+'T12:00');return d.toLocaleDateString('it-IT',{day:'numeric',month:'long',year:'numeric'})}
function enhanceFinanceControls(root=document){
  root.querySelectorAll('select:not([data-finance-enhanced])').forEach(select=>{
    select.dataset.financeEnhanced='1';
    const button=document.createElement('button');button.type='button';button.className=`finance-control-proxy finance-select-proxy ${select.className}`.trim();button.setAttribute('aria-label',select.getAttribute('aria-label')||'Seleziona opzione');
    const update=()=>{const o=select.options[select.selectedIndex];button.textContent=o?o.textContent:'Seleziona'};update();
    button.addEventListener('click',()=>openFinanceChoice(select,button));select.addEventListener('change',update);select.insertAdjacentElement('afterend',button);select.classList.add('finance-native-hidden');
  });
  root.querySelectorAll('input[type="date"]:not([data-finance-enhanced])').forEach(input=>{
    input.dataset.financeEnhanced='1';
    const button=document.createElement('button');button.type='button';button.className='finance-control-proxy finance-date-proxy';button.textContent=financeDateLabel(input.value);
    button.addEventListener('click',()=>openFinanceDate(input,button));input.addEventListener('change',()=>button.textContent=financeDateLabel(input.value));input.insertAdjacentElement('afterend',button);input.classList.add('finance-native-hidden');
  });
}
function refreshFinanceControl(control){if(!control)return;const proxy=control.nextElementSibling;if(proxy&&proxy.classList.contains('finance-control-proxy')){if(control.tagName==='SELECT'){const o=control.options[control.selectedIndex];proxy.textContent=o?o.textContent:'Seleziona'}else if(control.type==='date')proxy.textContent=financeDateLabel(control.value)}}
function openFinanceChoice(select,button){const root=document.createElement('div');root.className='finance-picker-backdrop';const options=[...select.options];root.innerHTML=`<div class="finance-picker"><div class="finance-picker-title">${escapeHtml(select.closest('.field')?.querySelector('label')?.textContent?.trim()||select.getAttribute('aria-label')||'Seleziona')}</div><div class="finance-choice-list">${options.map((o,i)=>`<button type="button" class="finance-choice ${o.selected?'selected':''}" data-index="${i}"><span>${escapeHtml(o.textContent)}</span><i></i></button>`).join('')}</div></div>`;const close=()=>root.remove();root.querySelectorAll('.finance-choice').forEach(choice=>choice.onclick=()=>{select.selectedIndex=Number(choice.dataset.index);select.dispatchEvent(new Event('change',{bubbles:true}));refreshFinanceControl(select);close()});root.addEventListener('click',e=>{if(e.target===root)close()});document.body.appendChild(root)}
function openFinanceDate(input,button){let value=input.value||todayIso();let [year,month]=value.split('-').map(Number);const root=document.createElement('div');root.className='finance-picker-backdrop';const renderCalendar=()=>{const selected=value;const first=new Date(year,month-1,1),last=new Date(year,month,0);const offset=(first.getDay()+6)%7;const cells=[];for(let i=0;i<offset;i++)cells.push('<span class="finance-date-empty"></span>');for(let day=1;day<=last.getDate();day++){const iso=`${year}-${String(month).padStart(2,'0')}-${String(day).padStart(2,'0')}`;cells.push(`<button type="button" class="finance-day ${iso===selected?'selected':''}" data-date="${iso}">${day}</button>`)}root.innerHTML=`<div class="finance-date-picker"><div class="finance-date-title">Seleziona data</div><div class="finance-date-value">${financeDateLabel(value)}</div><div class="finance-date-nav"><button type="button" data-nav="prev">‹</button><strong>${new Intl.DateTimeFormat('it-IT',{month:'long',year:'numeric'}).format(new Date(year,month-1,1))}</strong><button type="button" data-nav="next">›</button></div><div class="finance-weekdays"><span>L</span><span>M</span><span>M</span><span>G</span><span>V</span><span>S</span><span>D</span></div><div class="finance-calendar-grid">${cells.join('')}</div><div class="finance-date-actions"><button type="button" class="secondary-btn">Annulla</button><button type="button" class="primary-btn" data-ok>Ok</button></div></div>`;root.querySelector('[data-nav="prev"]').onclick=()=>{month--;if(month<1){month=12;year--}renderCalendar()};root.querySelector('[data-nav="next"]').onclick=()=>{month++;if(month>12){month=1;year++}renderCalendar()};root.querySelectorAll('.finance-day').forEach(day=>day.onclick=()=>{value=day.dataset.date;renderCalendar()});root.querySelector('.secondary-btn').onclick=()=>root.remove();root.querySelector('[data-ok]').onclick=()=>{input.value=value;input.dispatchEvent(new Event('change',{bubbles:true}));refreshFinanceControl(input);root.remove()}};renderCalendar();root.addEventListener('click',e=>{if(e.target===root)root.remove()});document.body.appendChild(root)}
