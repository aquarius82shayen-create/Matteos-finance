...
// Sostituisce esclusivamente le finestre native di selezione e calendario con popup coerenti con Matteo's Finance.
...
function openFinanceSelect(select){if(!select||select.disabled)return;const options=[...select.options].filter(o=>!o.disabled);const label=pickerLabelFor(select);const root=pickerRoot();root.innerHTML=`<div class="finance-picker-backdrop" onclick="if(event.target===this)closeFinancePicker()"><div class="finance-picker-card finance-select-picker"><div class="finance-picker-head"><div><small>${escapeHtml(label)}</small><h2>Seleziona un'opzione</h2></div><button type="button" class="finance-picker-close" onclick="closeFinancePicker()">×</button></div><div class="finance-option-list">${options.map((o,i)=>`<button type="button" class="finance-option ${o.value===select.value?'selected':''}" data-finance-option="${i}"><span>${escapeHtml(o.textContent.trim())}</span><i>${o.value===select.value?'✓':''}</i></button>`).join('')}</div></div></div>`;root.querySelectorAll('[data-finance-option]').forEach(btn=>btn.addEventListener('click',()=>{const option=options[Number(btn.dataset.financeOption)];if(!option)return;select.value=option.value;select.dispatchEvent(new Event('input',{bubbles:true}));select.dispatchEvent(new Event('change',{bubbles:true}));closeFinancePicker()}))}
...
function initFinancePickers(){if(document.body.dataset.financePickersReady)return;document.body.dataset.financePickersReady='1';document.addEventListener('pointerdown',event=>{const select=event.target.closest('select');const date=event.target.closest('input[type="date"]');if(!select&&!date)return;event.preventDefault();event.stopPropagation();if(select)openFinanceSelect(select);else openFinanceDate(date)},true);document.addEventListener('focusin',event=>{const select=event.target.closest?.('select');const date=event.target.closest?.('input[type="date"]');if(!select&&!date)return;if(document.getElementById('financePickerRoot')?.innerHTML)return;event.target.blur();if(select)openFinanceSelect(select);else openFinanceDate(date)},true)}
initFinancePickers();
