...
function semanticIconFile(description='',category='',type=''){
  ...
  const has=(...terms)=>terms.some(term=>{
    const t=String(term).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');
    return new RegExp(`(^|\\s)${t.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}(?=\\s|$)`,'u').test(text);
  });
  const phrase=(...terms)=>terms.some(term=>text.includes(
    String(term).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'')
  ));

  // Specific concepts first: an explicit object/activity in the description
  // always outranks a broad category or a place mentioned later.
  const rules=[
    [['lampadina','lampade','lampada','luce','luci'],ICONS.light],
    [['bilanciare entrate e uscite','bilanciare entrate','bilanciare le entrate','bilanciare il bilancio','pareggiare entrate e uscite','pareggiare le entrate e le uscite','bilancio'],ICONS.bilancia],
    [['concerto','concerti','live','spettacolo musicale'],ICONS.concert],
    [['iron maiden','metal','heavy metal'],ICONS.musica],
    [['benzina','carburante','rifornimento','rifornire'],ICONS.fuel],
    [['aereo','aereoporto','aeroporto','volo','volare'],ICONS.flight],
    [['treno','ferrovia','ferroviario'],ICONS.train],
    [['crociera','crociere','crociera'],ICONS.cruise],
    [['hotel','albergo','alloggio','b&b','bed and breakfast'],ICONS.hotel],
    [['campeggio','campeggio','tenda'],ICONS.camping],
    [['palestra','allenamento','fitness','gym'],ICONS.gym],
    [['medico','dottore','visita medica'],ICONS.doctor],
    [['dentista','dentista'],ICONS.dentist],
    [['farmacia','farmaci','medicinale','medicinali'],ICONS.pharmacy],
    [['veterinario','veterinaria'],ICONS.vet],
    [['ristorante','ristorazione'],ICONS.restaurant],
    [['cena','cenare'],ICONS.dinner],
    [['aperitivo','aperitivi'],ICONS.aperitivo],
    [['pizza','pizze'],ICONS.pizza],
    [['hamburger','burger'],ICONS.hamburger],
    [['matrimonio','nozze'],ICONS.wedding],
    [['natale','christmas'],ICONS.christmas],
    [['capodanno','anno nuovo'],ICONS.newYear],
    [['pasqua'],ICONS.easter],
    [['ferragosto'],ICONS.ferragosto],
    [['compleanno','festa','party'],ICONS.party],
    [['regalo','regali'],ICONS.gift],
    [['donazione','donazioni','beneficenza'],ICONS.charity],
    [['tasse','imposte','f24'],ICONS.taxes],
  ];

  // Exact/specific rules are evaluated before place/country rules.
  for(const [terms,file] of rules){
    if(terms.some(t=>phrase(t)))return file;
  }

  // Places/countries: use these only when no more specific activity/object
  // was found in the description.
  const places=[
    [['dublino','irlanda'],ICONS.irlanda],
    [['londra','inghilterra','regno unito','uk','united kingdom'],ICONS.uk],
    [['berlino','germania'],ICONS.germania],
    [['roma','italia'],ICONS.italia],
    [['madrid','spagna'],ICONS.spagna],
    [['parigi','francia'],ICONS.francia],
    [['lisbona','portogallo'],ICONS.portogallo],
    [['vienna','austria'],ICONS.austria],
    [['atene','grecia'],ICONS.grecia],
    [['istanbul','turchia'],ICONS.turchia],
    [['zagabria','croazia'],ICONS.croazia],
    [['praga','repubblica ceca'],ICONS.repubblicaCeca],
    [['budapest','ungheria'],ICONS.ungheria],
    [['varsavia','polonia'],ICONS.polonia],
    [['bratislava','slovacchia'],ICONS.slovacchia],
    [['lubiana','slovenia'],ICONS.slovenia],
    [['reykjavik','islanda'],ICONS.islanda],
    [['oslo','norvegia'],ICONS.norvegia],
    [['stoccolma','svezia'],ICONS.svezia],
    [['copenaghen','danimarca'],ICONS.danimarca],
    [['helsinki','finlandia'],ICONS.finlandia],
    [['vilnius','lituania'],ICONS.lituania],
    [['riga','lettonia'],ICONS.lettonia],
    [['tallinn','estonia'],ICONS.estonia],
    [['varsavia','polonia'],ICONS.polonia],
    [['bruxelles','belgio'],ICONS.belgio],
    [['bucharest','bucarest','romania'],ICONS.romania],
    [['sofia','bulgaria'],ICONS.bulgaria],
    [['malta'],ICONS.malta],
    [['messico'],ICONS.messico],
    [['stati uniti','usa','america'],ICONS.usa],
    [['giappone','tokyo','kyoto'],ICONS.giappone],
    [['cina','pechino','shanghai'],ICONS.cina],
    [['canada','toronto','vancouver'],ICONS.canada],
    [['thailandia','bangkok'],ICONS.thailandia],
    [['vietnam'],ICONS.vietnam],
    [['indonesia','bali'],ICONS.indonesia],
    [['india'],ICONS.india],
    [['singapore'],ICONS.singapore],
    [['marocco'],ICONS.marocco],
    [['egitto'],ICONS.egitto],
    [['sudafrica','sud africa'],ICONS.sudafrica],
    [['brasile'],ICONS.brasile],
    [['cile'],ICONS.cile],
    [['argentina'],ICONS.argentina],
    [['colombia'],ICONS.colombia],
    [['peru'],ICONS.peru],
    [['costa rica'],ICONS.costaRica],
    [['cuba'],ICONS.cuba],
    [['giamaica'],ICONS.giamaica],
    [['bahamas'],ICONS.bahamas],
    [['filippine'],ICONS.filippine],
    [['cambogia'],ICONS.cambogia],
    [['nuova zelanda'],ICONS.nuovaZelanda],
    [['fiji'],ICONS.fiji],
    [['porto rico'],ICONS.portoRico],
    [['hong kong'],ICONS.hongKong],
    [['malaysia'],ICONS.malaysia],
    [['australia'],ICONS.australia],
    [['valledoria','sardegna'],ICONS.italia],
  ];

  for(const [terms,file] of places){
    if(terms.some(t=>phrase(t)))return file;
  }

  // Generic activity/theme concepts, still based only on the description.
  const generic=[
    [['viaggio','viaggi','vacanza','vacanze','ferie','weekend','tour'],ICONS.travel],
    [['spiaggia','mare','balneare','bagno'],ICONS.beach],
    [['montagna','trekking','escursione'],ICONS.mountain],
    [['bar','pub'],ICONS.bar],
    [['teatro'],ICONS.theatre],
    [['auto','macchina','automobile'],ICONS.car],
    [['meccanico','riparazione auto','gommista'],ICONS.repair],
    [['guidare','guida'],ICONS.driving],
    [['casa','abitazione'],ICONS.house],
    [['elettrodomestico','elettrodomestici','frigorifero','lavatrice'],ICONS.appliances],
    [['gas'],ICONS.gas],
    [['internet','wifi','wi fi'],ICONS.internet],
    [['bolletta','bollette'],ICONS.bills],
    [['trasloco'],ICONS.moving],
    [['attrezzi','utensili'],ICONS.tools],
    [['amore','fidanzata','fidanzato','coppia'],ICONS.love],
    [['salute','benessere'],ICONS.health],
    [['famiglia'],ICONS.family],
    [['animale','animali','gatto','cane'],ICONS.pet],
    [['occhiali da sole','occhiali'],ICONS.sunglasses],
    [['shopping','acquisti','negozio'],ICONS.shopping],
    [['elettronica','computer','telefono','smartphone'],ICONS.electronics],
    [['obiettivo','obiettivi'],ICONS.target],
    [['portafoglio','soldi','denaro'],ICONS.wallet],
  ];

  for(const [terms,file] of generic){
    if(terms.some(t=>phrase(t)))return file;
  }
