...
function dashboardWeekDays(offset=dashboardWeekOffset){
  ...
  return Array.from({length:7},(_,i)=>{
    ...
    return {date,key,name:WEEK_LABELS[date.getDay()],day:date.getDate(),inc,out};
  });
}
...
return `<section class="page dashboard-page ${state.page==='dashboard'?'active':''}${dashboardWeekTransition?' week-changing':''}">
  <div class="hero">
    ...
    <img class="dashboard-logo" src="assets/dashboard-logo-red-2.png" alt="Matteo's Finance" />
    <div class="hero-actions"><button class="icon-btn" title="Notifiche">🔔</button><button class="icon-btn" onclick="openSettings()" title="Impostazioni">⚙</button></div>
    <select class="month-select" aria-label="Mese dashboard"><option>Settembre 2026</option></select>
    <div class="balance-card"><div class="balance-label-row"><div class="balance-label">TOTALE DISPONIBILE</div></div><div class="balance-value-row"><div class="balance-value">${euroFmt.format(state.balance)}</div><img class="balance-wallet" src="assets/Wallet-1.png" alt="Portafoglio" /></div><div class="balance-delta"><span class="positive">↗ ${euroFmt.format(sav)}</span> &nbsp; saldo del mese</div><div class="balance-buttons"><button class="big-btn income" onclick="openTransaction('income')">⊕ &nbsp; Aggiungi entrata</button><button class="big-btn expense" onclick="openTransaction('expense')">↟ &nbsp; Aggiungi uscita</button></div></div>
  </div>
  <div class="section"><div class="section-title week-title"><span>▣ SETTIMANA IN CORSO</span><span class="week-nav"><button type="button" onclick="shiftDashboardWeek(-1)" aria-label="Settimana precedente">‹</button><button type="button" onclick="shiftDashboardWeek(1)" aria-label="Settimana successiva">›</button></span></div><div class="panel week-panel"><div class="week-viewport">${dashboardWeekTransition?`<div class="week-track week-transition-${dashboardWeekTransition.direction}">${(dashboardWeekTransition.direction==='next'?[dashboardWeekDays(dashboardWeekTransition.from),days]:[days,dashboardWeekDays(dashboardWeekTransition.from)]).map((week,weekIndex)=>`<div class="week-pane"><div class="week-grid">${week.map(day=>`<div class="day-card ${day.key==='2026-09-06'&&((dashboardWeekTransition.direction==='next'?dashboardWeekTransition.from:dashboardWeekTransition.to)===0)&&weekIndex===0?'today':''}"><div class="day-name">${day.name}</div><div class="day-num">${day.day}</div><div class="day-in positive">+€ ${weekAmount(day.inc)}</div><div class="day-out negative">-€ ${weekAmount(day.out)}</div><button class="day-plus" onclick="openTransaction()">＋</button></div>`).join('')}</div></div>`).join('')}</div>`:`<div class="week-grid">${days.map(day=>`<div class="day-card ${day.key==='2026-09-06'&&dashboardWeekOffset===0?'today':''}"><div class="day-name">${day.name}</div><div class="day-num">${day.day}</div><div class="day-in positive">+€ ${weekAmount(day.inc)}</div><div class="day-out negative">-€ ${weekAmount(day.out)}</div><button class="day-plus" onclick="openTransaction()">＋</button></div>`).join('')}</div>`}</div></div></div>
  <div class="stats-grid"><div class="stat-card green"><b><span class="stat-icon">↗</span> ENTRATE</b><strong>${euroFmt.format(inc)}</strong><small>Questo mese</small></div><div class="stat-card pink"><b><span class="stat-icon">↘</span> USCITE</b><strong>${euroFmt.format(out)}</strong><small>Questo mese</small></div><div class="stat-card blue"><b><span class="stat-icon coin-icon">€</span> RISPARMIO</b><strong>${euroFmt.format(sav)}</strong><small>Questo mese</small></div></div>
...
function submitTransaction(ev){ev.preventDefault();const f=ev.currentTarget,d=new FormData(f),amount=Number(d.get('amount'));state.transactions.push({id:uid(),date:d.get('date'),type:f.dataset.type,category:d.get('category'),description:d.get('description'),amount});recalculateBalance();save();closeModal();render()}
