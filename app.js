...
const expenseCategories=['Spese','Benzina','Casa','Affitto','Contrattempo','Tempo libero','Altro'];
...
function openFutureEvent(id=null){
  const existing=id?state.futureEvents.find(e=>e.id===id):null;
  let type=existing?existing.type:'expense';
  const date=existing?existing.date:`${state.forecastMonth}-15`;
  const description=existing?escapeHtml(existing.description):'';
  const amount=existing?existing.amount:'';
  const actionLabel=existing?'Salva modifiche':'Salva evento';
  openModal(`<div class="modal-head"><h2>${existing?'Modifica evento previsto':'Nuovo evento previsto'}</h2><button class="close-btn" onclick="closeModal()">×</button></div><div class="type-switch"><button id="typeIncome" class="income ${type==='income'?'active':''}" onclick="selectFutureType('income')">↓ Entrata</button><button id="typeExpense" class="expense ${type==='expense'?'active':''}" onclick="selectFutureType('expense')">↑ Uscita</button></div><form id="futureForm" class="form-grid"><div class="form-grid two"><div class="field"><label>Data</label><input name="date" type="date" value="${date}" required></div><div class="field"><label>Categoria</label><select id="futureCategory" name="category">${futureCategoryOptions(type)}</select></div></div><div class="field"><label>Descrizione</label><input name="description" value="${description}" required></div><div class="field"><label>Importo</label><input name="amount" type="number" min="0.01" step="0.01" value="${amount}" required></div><div class="modal-actions"><button type="button" class="secondary-btn" onclick="closeModal()">Annulla</button><button class="primary-btn">${actionLabel}</button></div></form>`);
  const form=document.getElementById('futureForm');form.dataset.type=type;if(existing)form.dataset.editId=existing.id;
  const category=document.getElementById('futureCategory');if(existing&&category)category.value=existing.category;
  form.addEventListener('submit',submitFuture);
}
...
function openGoal(id=null){
  ...
  let priority=existing?existing.priority:'media', cushionEnabled=existing?(existing.cushion||0)>0:false;
  openModal(`<div class="modal-head"><h2>${existing?'Modifica obiettivo':'Nuovo obiettivo'}</h2><button class="close-btn" onclick="closeModal()">×</button></div><form id="goalForm" class="form-grid"><div class="field"><label>Quale meta vuoi raggiungere?</label><input name="name" value="${existing?escapeHtml(existing.name):''}" placeholder="Es. Chitarra nuova" required></div><div class="field"><label>Quanto costa?</label><input name="cost" type="number" min="1" step="0.01" value="${existing?existing.cost:''}" required></div><div class="field"><label>Entro quando?</label><input name="deadline" type="date" value="${existing?existing.deadline:'2026-12-31'}" required></div><div class="field"><label>Dai una priorità a questo obiettivo</label><div class="priority-switch"><button type="button" data-p="bassa" class="${priority==='bassa'?'active':''}">♧ Bassa</button><button type="button" data-p="media" class="${priority==='media'?'active':''}">◉ Media</button><button type="button" data-p="alta" class="${priority==='alta'?'active':''}">♥ Alta</button></div></div><div class="field"><label>Vuoi fissare un minimo di cuscino?</label><div class="type-switch"><button type="button" id="cushionNo" class="${!cushionEnabled?'active':''}">No</button><button type="button" id="cushionYes" class="${cushionEnabled?'active':''}">Sì</button></div></div><div id="cushionField" class="field" style="display:${cushionEnabled?'block':'none'}"><label>Importo minimo del cuscino</label><input name="cushion" type="number" min="0" step="0.01" value="${existing?(existing.cushion||0):0}"></div><div class="modal-actions"><button type="button" class="secondary-btn" onclick="closeModal()">Annulla</button><button class="primary-btn">${existing?'Salva modifiche':'Analizza obiettivo'}</button></div></form>`);
  document.querySelectorAll('[data-p]').forEach(b=>b.onclick=()=>{priority=b.dataset.p;document.querySelectorAll('[data-p]').forEach(x=>x.classList.toggle('active',x===b))});
  document.getElementById('cushionNo').onclick=()=>{cushionEnabled=false;document.getElementById('cushionField').style.display='none';document.getElementById('cushionNo').classList.add('active');document.getElementById('cushionYes').classList.remove('active')};
  document.getElementById('cushionYes').onclick=()=>{cushionEnabled=true;document.getElementById('cushionField').style.display='block';document.getElementById('cushionYes').classList.add('active');document.getElementById('cushionNo').classList.remove('active')};
  document.getElementById('goalForm').addEventListener('submit',ev=>{ev.preventDefault();const d=new FormData(ev.currentTarget),data={name:d.get('name'),cost:Number(d.get('cost')),deadline:d.get('deadline'),priority,cushion:cushionEnabled?Number(d.get('cushion')):0};if(existing){const i=state.goals.findIndex(g=>g.id===existing.id);if(i>=0)state.goals[i]={...state.goals[i],...data};save();closeModal();render()}else{const g={id:uid(),...data,saved:0};state.goals.push(g);save();closeModal();openGoalAnalysis(g.id)}})
}
