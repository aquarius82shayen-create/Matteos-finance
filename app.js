...
return `<section class="page dashboard-page ${state.page==='dashboard'?'active':''}${dashboardWeekTransition?' week-changing':''}">
  ...
  <div class="section"><div class="section-title week-title"><span>${new Date(ym+'-01T12:00:00').toLocaleDateString('it-IT',{month:'long'}).toUpperCase()}</span><span class="week-nav"><button type="button" onclick="shiftDashboardWeek(-1)" aria-label="Giorni precedenti">‹</button><button type="button" onclick="shiftDashboardWeek(1)" aria-label="Giorni successivi">›</button></span></div><div class="panel week-panel"><div class="week-viewport">${dashboardWeekTransition?`<div class="week-track week-transition-${dashboardWeekTransition.direction}">${(dashboardWeekTransition.direction==='next'?[dashboardWeekDays(dashboardWeekTransition.from),days]:[days,dashboardWeekDays(dashboardWeekTransition.from)]).map(week=>`<div class="week-pane"><div class="week-grid">${week.map(day=>dayCardMarkup(day,day.key===todayIso())).join('')}</div></div>`).join('')}</div>`:`<div class="week-grid">${days.map(day=>dayCardMarkup(day,day.key===todayIso())).join('')}</div>`}</div></div></div>
