...
function openFinanceDate(input){if(!input||input.disabled)return;const root=pickerRoot();const isGoalDate=input.dataset.goalDate==='1';const hidden=isGoalDate?input.form?.querySelector(`input[type="hidden"][name="${input.dataset.dateName}"]`):null;let selected=isGoalDate?(hidden?.value||input.dataset.dateValue||''):(input.value||'');let initial=selected?new Date(selected+'T12:00:00'):new Date();if(isGoalDate&&input.dataset.dateName==='deadline'){const start=input.form?.querySelector('input[type="hidden"][name="startDate"]')?.value;if(start)initial=new Date(start+'T12:00:00')}let viewYear=initial.getFullYear(),viewMonth=initial.getMonth();const label=pickerLabelFor(input);const renderCalendar=()=>{const first=new Date(viewYear,viewMonth,1),days=new Date(Date.UTC(viewYear,viewMonth+1,0)).getUTCDate(),start=(new Date(Date.UTC(viewYear,viewMonth,1)).getUTCDay()+6)%7;let cells='';for(let i=0;i<start;i++)cells+='<span class="finance-day empty"></span>';for(let day=1;day<=days;day++){const iso=`${viewYear}-${pad2(viewMonth+1)}-${pad2(day)}`;cells+=`<button type="button" class="finance-day ${iso===selected?'selected':''}" data-finance-date="${iso}">${day}</button>`}root.innerHTML=`<div class="finance-picker-backdrop" onclick="if(event.target===this)closeFinancePicker()"><div class="finance-picker-card finance-date-picker"><div class="finance-picker-head"><div><small>${escapeHtml(label)}</small><h2>${selected?new Date(selected+'T12:00:00').toLocaleDateString('it-IT',{day:'numeric',month:'long',year:'numeric'}):'Seleziona una data'}</h2></div><button type="button" class="finance-picker-close" onclick="closeFinancePicker()">×</button></div><div class="finance-calendar-nav"><button type="button" data-finance-prev>‹</button><b>${monthTitle(viewYear,viewMonth)}</b><button type="button" data-finance-next>›</button></div><div class="finance-weekdays"><span>L</span><span>M</span><span>M</span><span>G</span><span>V</span><span>S</span><span>D</span></div><div class="finance-calendar-grid">${cells}</div><div class="finance-picker-actions"><button type="button" class="secondary-btn" onclick="closeFinancePicker()">Annulla</button><button type="button" class="primary-btn" data-finance-today>Oggi</button></div></div></div>`;root.querySelector('[data-finance-prev]').onclick=()=>{viewMonth--;if(viewMonth<0){viewMonth=11;viewYear--}renderCalendar()};root.querySelector('[data-finance-next]').onclick=()=>{viewMonth++;if(viewMonth>11){viewMonth=0;viewYear++}renderCalendar()};root.querySelectorAll('[data-finance-date]').forEach(btn=>btn.onclick=()=>{const iso=btn.dataset.financeDate;if(isGoalDate){input.dataset.dateValue=iso;if(hidden)hidden.value=iso;input.value=new Date(iso+'T12:00:00').toLocaleDateString('it-IT',{day:'2-digit',month:'2-digit',year:'numeric'});input.dispatchEvent(new Event('input',{bubbles:true}));input.dispatchEvent(new Event('change',{bubbles:true}))}else{input.value=iso;input.dispatchEvent(new Event('input',{bubbles:true}));input.dispatchEvent(new Event('change',{bubbles:true}))}selected=iso;closeFinancePicker()});root.querySelector('[data-finance-today]').onclick=()=>{const iso=todayIso();if(isGoalDate){input.dataset.dateValue=iso;if(hidden)hidden.value=iso;input.value=new Date(iso+'T12:00:00').toLocaleDateString('it-IT',{day:'2-digit',month:'2-digit',year:'numeric'});input.dispatchEvent(new Event('input',{bubbles:true}));input.dispatchEvent(new Event('change',{bubbles:true}))}else{input.value=iso;input.dispatchEvent(new Event('input',{bubbles:true}));input.dispatchEvent(new Event('change',{bubbles:true}))}selected=iso;closeFinancePicker()}};renderCalendar()}
function initFinancePickers(){if(document.body.dataset.financePickersReady)return;document.body.dataset.financePickersReady='1';document.addEventListener('click',event=>{const date=event.target.closest?.('input[type="date"]');if(!date)return;event.preventDefault();event.stopImmediatePropagation();openFinanceDate(date)},true)}
initFinancePickers();
