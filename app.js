...
function dayCardMarkup(day,today=false){return `<div class="day-card ${today?'today':''}"><div class="day-name">${day.name}</div><div class="day-num">${day.day}</div><div class="day-month">${day.month}</div><div class="day-in positive">+€ ${weekAmount(day.inc)}</div><div class="day-out negative">-€ ${weekAmount(day.out)}</div><button class="day-plus" onclick="openTransaction()">＋</button></div>`}
