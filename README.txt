...
Pacchetto 61: il motore delle icone legge dinamicamente la cartella assets/icons del repository GitHub tramite la directory pubblica del repository. La libreria incorporata nel codice resta disponibile come fallback offline. I nomi descrittivi dei file WEBP vengono usati come vocabolario per il riconoscimento: entita/nome specifico -> luoghi -> concetti -> freccia.

GESTIONE DINAMICA DELLA LIBRERIA ICONE
---------------------------------------
La lista delle icone non è più vincolata esclusivamente a un elenco scritto nel codice. All'avvio l'app prova a leggere i file WEBP presenti nella cartella assets/icons del repository GitHub corrente.

Questo permette di aggiungere nuove icone, eliminare icone o rinominare un file WEBP direttamente nella cartella assets/icons senza dover modificare app.js. Il nuovo nome del file viene utilizzato come vocabolario per i successivi riconoscimenti.

Esempio:
41_casa_affitto_canone_appartamento.webp

Una descrizione contenente "affitto" o "canone" può quindi essere associata automaticamente a quella icona, purché non esista una corrispondenza più specifica secondo la gerarchia del motore.

Se GitHub non è raggiungibile, l'app utilizza la copia locale/fallback della libreria già conosciuta e continua a funzionare. La lista remota viene memorizzata temporaneamente sul dispositivo per evitare richieste ripetute.

NOTA: il repository deve mantenere le icone nella cartella assets/icons e i file devono avere estensione .webp.
